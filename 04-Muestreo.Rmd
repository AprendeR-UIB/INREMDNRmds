...
Esto nos permite entender que, cuando la población es muchísimo más grande que la muestra,  los muestreos aleatorios con y sin reposición son equivalentes en el sentido siguiente: puesto que si la población es muy, muy grande, un muestreo con reposición daría casi seguro una muestra con todos sus elementos diferentes, podemos tomar directamente la muestra sin reposición y aceptar que permitíamos repeticiones, pero que no se han dado, y que por tanto la muestra es simple. 
...
* Pero en cambio ya es difícil de creer que una muestra aleatoria de 100 estudiantes diferentes de la UIB haya sido tomada permitiendo repeticiones, porque en una de cada tres muestras tomadas permitiendo repeticiones nos saldría alguna repetición: hay que ser muy optimistas para estar "casi seguros" que la nuestra es una de las otras doas.
...
Este método de muestreo se utiliza cuando la población está clasificada en  **estratos** que son de interés para la propiedad estudiada. Estos estratos serán grupos de individuos definidos por una característica concreta, de manera que individuos del mismo estrato tengan esa catacterística en común (los estratos sean **homogéneos** internamente) y individuos de estratos diferentes tengan esa característica diferente (los estratos sean **heterogéneos** externamente).
...
* **Muestreo por conglomerados**:
    ...
    * Los clústers forman una clasificación de los sujetos de la población en grupos pequeños de individuos definidos por una característica que en principio es irrelevante en el estudio estadístico. Por ejemplo, la manzana donde vive o el pediatra al que está asignado.

    * Se escogen algunos clústers al azar y se incluyen en la muestra todos sus elementos.
...
```{block2,type="rmdnote"}
La diferencia  los clústers pueden ser heterogéneos, y de hecho conviene que lo sean, respecto de las características 
...
Como véis, el muestreo tuvo dos etapas. En la primera se seleccionaron clústers mediante un muestreo no aleatorio estratificado: los estratos fueron los 14 sectores geográficos, los clústers elegidos estaban formados por los grupos de pacientes adscritos a los médicos de familia que voluntariamente participaron en el estudio. La segunda fue un muestreo aleatorio dentro de cada clúster.
