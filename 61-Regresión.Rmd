...
Aunque la regresión lineal simple es un caso particular de la regresión lineal múltiple, empezaremos tratando en detalle la primera ya que es la más sencilla. Por otro lado, de la regresión lineal generalizada solo explicaremos uno de los tipos más usados en medicina: una variable binaria como función lineal de varias variables también binarias, los que nos permitirá calcular **odds ratios ajustadas**: la OR de la variable dependiente relativa a cada una de las variables independientes *tras descontar el efecto de las otras variables independientes.*
...
En la Sección \@ref(sec:edbivar) añadíamos a un gráfico como este la *recta de regresión* que "más se aproxima" a los puntos:
...
* $\beta_1$ es la variación esperada del valor de $Y$ cuando el valor de $X$ aumenta en 1 unidad

Tomamos una muestra $(x_i,y_i)_{i=1,\ldots,n}$ de $n$ valores del par de variables $(X,Y)$ y con ella obtenemos estimaciones $b_0,b_1$ de  $\beta_0$ y $\beta_1$. Estas estimaciones definen la **recta de regresión** de $Y$ en función de $X$
$$
y=b_0+b_1 x
$$


Esta recta sirve, por ejemplo, para, dado un valor $x_0$ de $X$, estimar qué vale $Y$ en los individuos en los que $X$ vale $x_0$. Denotaremos por $\widehat{y}$ el valor **estimado** de $Y$ en un sujeto cuyo valor de $X$ sea  $x$ usando la recta de regresión:
$$
\widehat{y}=b_0+b_1 x
$$ 

Hay varias técnicas para estimar los coeficientes $\beta_0,\beta_1$. Aquí vamos a explicar la más popular: la **regresión lineal por mínimos cuadrados**. La idea es que tomamos como $y=b_0+b_1 x$  la recta que minimiza su suma de cuadrados de distancias verticales desde los puntos de la muestra.

En concreto, dados $b_0,b_1$, llamamos el  **error** del modelo $\widehat{y}=b_0+b_1 x$ en el punto $i$-ésimo de la muestra $(x_i,y_i)$ a la diferencia $e_i$ entre el valor  $y_i$ de $Y$ en este punto y su valor  estimado $\widehat{y}_i$:
$$
...
$$
\mu_{\text{VEF1}|\text{Altura}}=-9.1904+  0.0744\cdot \text{Altura}
$$
...
Según el modelo lineal obtenido, el valor esperado de VEF1 cuando $\text{Altura}=175$ es $-9.1904+0.0744\cdot 165=3.086$. Aunque naturalmente, a estas alturas hemos de tener claro que hay que acompañar esta estimación puntual con un intervalo de confianza. Los estudiaremos en la Sección \@ref(sec:confintrls).
...
El motivo por el que se usa $R^2$ y no $|r_{x,y}|$ es que, calculando un poco, se obtiene que, en la regresión por mínimos cuadrados, el coeficiente de determinación es igual a *la varianza de los valores estimados* de $Y$ sobre la muestra usada dividida por *la varianza de los valores observados* de $Y$ en la muestra:
$$
...
Como siempre, para poder usar fórmulas paramétricas para calcular intervalos de confianza necesitamos que se verifiquen algunas condiciones sobre las variables involucradas. En este caso, las condiciones que vamos a suponer que se cumplen son las siguientes. Para cada $i=1,\ldots,n$, sea $E_i=Y|x_i-\beta_0-\beta_1 x_i$, a la que llamaremos la  **variable aleatoria error $i$-ésima**: esta variable toma un sujeto en el que $X$ valga $x_i$ y calcula la diferencia entre su valor de $Y$ y su valor esperado de $Y$ (que, recordemos, vale $\beta_0+\beta_1 x_i$). Supongamos que estas variables error satisfacen las condiciones siguientes:

* Todas son normales de media 0
* Todas tienen la misma varianza: $\sigma_{E_i}=\sigma_{E_j}$ para todos $i\neq j$.
* Son incorreladas dos a dos: $\sigma_{E_i,E_j}=0$ para todos $i\neq j$
...
*  Entre todos los estimadores insesgados de $\beta_0$ y $\beta_1$, $b_0$ y $b_1$ son los que tienen menor error típico: son los estimadores insesgados **más precisos**

*  Unos  estadísticos relacionados con $b_0$ y $b_1$ (y que no vamos a dar, tranquilos) tienen distribuciones t de Student conocidas, lo que permite calcular intervalos de confianza para $\beta_0$, $\beta_1$ y $\mu_{Y|x_0}$ centrados en sus estimaciones $b_0$, $b_1$ e $\widehat{y}$


Con JAMOVI podéis comprobar si se puede aceptar que las variables error $E_i$ cumplen las suposiciones que hemos hecho para que el cálculo de intervalos de confianza tenga sentido en  **Regresión/Regresión lineal/Comprobaciones de supuestos**. En la regresión lineal simple nos interesan la "Prueba de normalidad" y la "Prueba de autocorrelación":
...
Los p-valores del test de normalidad y del test de autocorrelación son grandes, por lo que podemos aceptar que todas la variables $E_i$ tienen la misma distribución normal de media 0 y que están incorreladas. La conclusión sobre la normalidad de los residuos la hemos ilustrado con un q-q-plot de los errores $e_i$.
...
* Definir una tabla de datos con el valor (o los valores) de la variable independiente `X` para el que queramos calcular el intervalo de confianza del valor esperado de la variable dependiente. La llamaremos `X.nuevas`.
...
Como veis, la predicción del VEF1 de Pep es mucho más incierta que la de su VEF1 "esperado": es más difícil acertar el VEF1 de un joven concreto que el valor medio del VEF1 de los jóvenes de su altura.
...
Cuando $k=1$, estamos ante un modelo de regresión lineal simple, así que en realidad la regresión simple es un caso particular de regresión múltiple.
Y, como en el caso simple:

* $\beta_0$ es el valor esperado de $Y$  en los individuos en los que $X_1=\cdots=X_k=0$

* Cada $\beta_i$, $i=1,\ldots,k$, es la variación esperada del valor de $Y$  cuando el valor de  $X_i$ aumenta en 1 unidad y el resto de las variables independientes permanecen constantes.
...
$$
de $(X_1,\ldots,X_k,Y)$ sobre una muestra de la población.  Para simplificar  la notación, a veces denotaremos el vector de valores de las variables independientes en el sujeto $i$-ésimo de la muestra, $(x_{i,1},x_{i,2},\ldots,x_{i,k})$, por $\underline{x}_i$.
...
La columna "Estimador" de la tabla que resulta contiene los coeficientes estimados de la función de regresión lineal. Da:
$$
...
$$
\begin{array}{rl}
\text{Peso esperado}\ =\!\!\!\!\! & -5417.14+11.18\cdot \text{Edad.madre}\\
&\quad+5.37\cdot \text{Peso.madre} -1.68\cdot\text{Tensión.sistólica}\\
&\quad +8.69\cdot\text{Nivel.colesterol} +166.07\cdot\text{Edad.gestacional}
\end{array}
$$

A partir de este modelo podemos predecir el aumento esperado del peso  neonatal asociado al aumento en una unidad de cada variable independiente si las otras variables permanecen constantes. Por ejemplo, a igualdad de los valores del resto de variables independientes, el aumento de 1 mg/dl en el nivel de colesterol basal está asociado a un aumento esperado de 8.7 gramos del neonato. 
...
*  La función lineal de regresión pasa por el vector de medias muestrales  $(\overline{x}_1,\overline{x}_2,\ldots,\overline{x}_k,\overline{y})$:
$$
...
<!--
Por cierto, con JAMOVI podemos efectuar simultáneamente varias regresiones lineales de la misma variable dependiente, para facilitar la comparación de los resultados y sus medidas de ajuste del modelo. Para ello hay que ir a **Constructor del Modelo** y usar "Añadir nuevo bloque" para definir nuevos subconjuntos de variables independientes:
  


```{r jamoviRLM3.1,echo=FALSE,out.width="100%"}
knitr::include_graphics("INREMDN_files/figure-html/jamoviRLM3.png")
```



```{r jamoviRLM3.2,echo=FALSE,out.width="100%"}
knitr::include_graphics("INREMDN_files/figure-html/jamoviRLM3.png")
```
...
Como en el caso simple, para cada $i=1,\ldots,n$, la  **variable aleatoria error $i$-ésima** $E_i$ es la que toma un sujeto en el que $(X_1,\ldots,X_k)$ vale $\underline{x}_i$ y mide la diferencia entre su valor de $Y$ y su valor esperado de $Y$:
$$
E_{i}=Y|\underline{x}_{i}-(\beta_0+\beta_1 x_{i,1}+\cdots+\beta_k x_{i,k})
$$
Si, como en la regresión simple, estas variables error satisfacen las condiciones siguientes:
...
Por lo que refiere a los intervalos de confianza para las predicciones de la variable dependiente en función de las independientes, se pueden calcular con la función `predict` en la ventana del editor de R como en el caso simple. Por ejemplo, supongamos que queremos predecir el peso del bebé de una parturienta de 31 años que pesa 72 kg, con nivel de colesterol inicial 182 mg/dl y tensión sistólica media durante el embarazo 120 si la edad gestacional en el momento del parto es exactamente de 36 semanas.
...
Por otro lado, podemos usar las "estadísticas de colinealidad" que obtenemos marcando la casilla correspondiente en **Regresión/Regresión lineal/Comprobaciones de Supuestos** para valorar si alguna variable independiente es función lineal de las otras o no. En el caso de serlo, esa variable es redundante en el modelo lineal.

En concreto, el valor de VIF (*Variance Inflation Factor*) de la variable $X_i$ es
$$
\frac{1}{1-R^2_{i}}
$$
donde $R^2_{i}$ indica el $R^2$ de la regresión lineal de esa variable $X_i$, tomándola como dependiente, respecto de las otras variables independientes. Un valor del VIF muy cercano a 1 indica un $R^2_{i}$ muy cercano a 0 y por lo tanto que esa variable no se aproxima a una combinación lineal de las otras variables independientes. Es el caso de nuestro ejemplo. En cambio, un valor muy grande del VIF significaría un valor del $R^2_{i}$ muy cercano a 1 y por lo tanto que esa variable es aproximadamente combinación lineal del resto de variables independientes. En este caso podríamos eliminarla de la regresión, ya que sería redundante y no aportaría información nueva en la estimación de la $Y$. 
  
Volviendo a los p-valores de los contrastes $\beta_i=0$ contra $\beta_i\neq 0$, si no
hay ningún $\beta_i$, con $i=1,\ldots,k$, que podamos rechazar que  sea 0, el modelo de regresión lineal no tiene sentido:  $\beta_1=\cdots=\beta_k=0$ 
significaría que  el valor esperado de $Y$ en un sujeto no depende de sus valores de $X_1,\ldots,X_k$.

Pero claro, eso son $k$ contrastes, y ya sabemos que la probabilidad de cometer en alguno un error de tipo I crece con $k$. Además los contrastes no son independientes, ya que los realizamos todos con la misma muestra, así que ajustar los p-valores no es tan sencillo como multiplicarlos por $k$.
...
H_1:\text{no es verdad que } \mu_{Y|\underline{x}_1}=\cdots=\mu_{Y|\underline{x}_k}
\end{array}
\right.
$$
podemos rechazar la hipótesis nula, obtenemos evidencia de que no es verdad que $\beta_1=\beta_2=\cdots=\beta_k=0$ y por lo tanto obtenemos evidencia de que el modelo lineal tiene sentido.
