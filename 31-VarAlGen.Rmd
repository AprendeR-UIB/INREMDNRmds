...
Hemos comentado en la sección anterior que el módulo **distrACTION** de JAMOVI no conoce la distribución hipergeométrica. Pero `R` sí, y podéis usarlo en la ventana del editor de `R`. 

Para `R` la distribución hipergeométrica es `hyper` (la binomial es `binom`). Para cada distribución implementada en `R`:
