...
$$
son independientes. Es decir, cuando el valor que toma una de estas variables sobre un sujeto no afecta para nada los valores que toman las otras.
...
* Veamos su **función de distribución** $F_X$. Recordad que $F_X(x)=P(X\leqslant x)$ y que nuestra variable solo puede tomar los valores 0, 1, 2 y 3.
    ...
    * Si $0\leqslant x<1$, $F_X(x)=P(X\leqslant x)=P(X=0)=f_X(0)=1/8$, porque si $0\leqslant x<1$, el único valor $\leqslant x$ que puede tomar $X$ es el 0.

    * Si $1\leqslant x<2$, $F_X(x)=P(X\leqslant x)=P(X=0\text{ o }X=1)$ $=f_X(0)+f_X(1)=4/8=1/2$, porque si $1\leqslant x<2$, los únicos valores $\leqslant x$ que puede tomar $X$ son 0 y 1.

    * Si $2\leqslant x<3$, $F_X(x)=P(X\leqslant x)=P(X=0\text{ o }X=1\text{ o }X=2)$ $=f_X(0)+f_X(1)+f_X(2)=7/8$, porque si $2\leqslant x<3$, los únicos valores $\leqslant x$ que puede tomar $X$ son 0, 1 y 2.

    * Si $3\leqslant x$, $F_X(x)=P(X\leqslant x)=1$, porque si $3\leqslant x$, seguro que obtenemos un número de caras $\leqslant x$.
...
```{block2,type="rmdcaution"} 
Fijaos en que $E(X^2) \neq E(X)^2$. En los dos últimos ejemplos hemos visto que si $X$ es la variable aleatoria que cuenta el número de caras en 3 lanzamientos de una moneda equilibrada, $E(X^2)=3$ pero $E(X)^2=1.5^2=2.25$.
...
Sea $p\in [0,1]$. El **cuantil de orden $p$** (o **$p$-cuantil**) de una variable aleatoria $X$ discreta es el valor $x_p\in D_X$ tal que $P(X\leqslant x_p)\geqslant p$ pero $P(X< x_p)<p$. Es decir, es el menor valor de su dominio tal que $P(X\leqslant x_p)\geqslant p$.
...
* Sus propiedades básicas, como por ejemplo cuáles son sus parámetros, cuál es su valor esperado y si su densidad es simétrica o tiene una cola a algún lado.
...
* Pedir a una persona si la estadística le aburre:
    ...
    * $p$: La proporción de persones a las que aburre la estadística
    


```{r cyanide,echo=FALSE,out.width="80%", fig.cap="¿Con cuál te identificas?" }
...
```{block2,type="rmdimportant"}
Es una tontería, pero, por si acaso, fijaos en que si $X$ es $B(n,p)$, $P(X=k)$ no solo depende de $k$ sino también de los parámetros $n$ y $p$. Esto será general. Todas las variables aleatorias de una misma familia tienen la función de densidad de la misma forma, y solo varían los valores de los parámetros.
```
...
* Si lanzamos 20 veces un dado equilibrado, ¿cuál es la probabilidad de sacar como máximo 5 unos? Con las notaciones anteriores, nos piden $P(X\leqslant 5)$, y esta probabilidad nos la da la función de distribución de $X$. Es $F_X(5)$:
```{r}
pbinom(5,20,1/6)
```

* Si lanzamos 20 veces un dado equilibrado, ¿cuál es la probabilidad de sacar menos de 5 unos? Con las notaciones anteriores, nos piden $P(X< 5)$, es decir, $P(X\leqslant 4)=F_X(4)$:
```{r}
pbinom(4,20,1/6)
```


* Si lanzamos 20 veces un dado equilibrado, ¿cuál es la probabilidad de sacar 5 unos o más? Con las notaciones anteriores, nos piden $P(X\geqslant 5)$. Como lo contrario de sacar 5 unos o más es sacar 4 unos o menos, tenemos que $P(X\geqslant 5)=1-P(X\leqslant 4)=1-F_X(4)$:
```{r}
1-pbinom(4,20,1/6)
```


* Si lanzamos 20 veces un dado equilibrado, ¿cuál es el menor número $N$ de unos para el que la probabilidad de sacar como máximo $N$ unos llega al 25%? Nos piden el menor valor $N$ tal que $P(X\leqslant N)\geqslant 0.25$, y esto por definición es el 0.25-cuantil de $X$:
```{r}
qbinom(0.25,20,1/6)
```
...
```{example}
Recordad uno de los ejemplos de variables no binomiales de la sección anterior. En una aula hay 5 chicos y 45 chicas. Escojo 10 estudiantes, uno tras otro y sin repetirlos, para hacerles una pregunta. Cada elección es independiente de las otras. Cuento cuántos chicos he interrogado. Se trata de una variable hipergeométrica $H(5,45,10)$.
...
$$
que es la varianza de una variable $B(n,p)$ multiplicada por un factor de corrección debido al hecho de que ahora tomamos muestras sin repetición y la varianza es más pequeña que si las tomamos con repetición. A la raíz cuadrada de este factor,
$$
...
1. Si tomáis las muestras sin permitir individuos repetidos, los resultados salen más variados que si las tomáis permitiendo repeticiones. 
1. Si tomáis las muestras sin permitir individuos repetidos, los resultados salen menos variados que si las tomáis permitiendo repeticiones. 
1. Si tomáis las muestras permitiendo que salgan individuos repetidos, cuanto más grandes tomáis las muestras más variados salen los resultados.
1. Si tomáis las muestras permitiendo que salgan individuos repetidos, cuanto más grandes tomáis las muestras menos variados salen los resultados.
1. Si tomáis las muestras sin permitir individuos repetidos, cuanto más grandes tomáis las muestras más variados salen los resultados.
1. Si tomáis las muestras sin permitir individuos repetidos, cuanto más grandes tomáis las muestras menos variados salen los resultados.
1. Ninguna de las otras afirmaciones es verdadera.
