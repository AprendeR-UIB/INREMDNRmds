...
En un estudio estadístico inferencial, se toma una **muestra** de individuos de una **población** y se estiman algunas características de la población a partir de las de la muestra. Para que esto tenga sentido, es necesario que la muestra sea razonablemente representativa de la población, pero sin conocer las características de la población, no podemos saber si una muestra es representativa o no. 

Para salir de este *impasse*, la solución comúnmente aceptada es tomar una muestra **aleatoria**, es decir, escogiendo sus sujetos de alguna manera al azar. De esta manera:

* Al evitar preferencias en la elección, es más probable que la muestra sea representativa de la población (pero, naturalmente, no está garantizado).

* Podremos usar técnicas estadísticas que nos permitirán acotar errores en la estimación y la probabilidad de  cometerlos.
...
Cuando **el tamaño de la población es muy grande en relación al de la muestra**, como suele ser en caso en medicina, la probabilidad de que haya repeticiones en una muestra aleatoria simple es muy pequeña. Por ejemplo, la probabilidad de que, si escogemos 100 individuos de la Baleares al azar permitiendo repeticiones, se escoja más de una vez el mismo individuo es de 0.005.
...
Una manera muy sencilla de obtener una muestra de una población cuando disponemos de una lista ordenada de sus individuos es tomarlos a intervalos constantes: cada quinto individuo, cada décimo individuo... Podemos añadir una componente aleatoria escogiendo al azar el primer individuo que elegimos, y a partir del cual empezamos a contar. A esta técnica se la llama **muestreo sistemático**, **aleatorio** si además el primer sujeto se escoge de manera aleatoria. 
...
La Figura \@ref(fig:sist) describe una muestra aleatoria sistemática de 15 bolas de nuestra urna de 100 bolas: hemos empezado a escoger por la bola roja oscura, que ha sido elegida al azar, y a partir de ella hemos "reclutado" las bolas a intervalos de 7, volviendo al principio cuando hemos llegado al final de la lista de bolas
...
La ventaja del muestreo aleatorio estratificado respecto del "transversal" es que, al facultar obtener una muestra de cada estrato de la población del tamaño que quiera el investigador:

1. Permite estimar la información de interés para cada estrato por separado, como si se tratara de un estudio diferente. 

2. Permite estimar la información sobre subpoblaciones minoritarias que en una muestra aleatorio transversal aparecerían  subrepresentadas.
...
El proceso de obtener y estudiar una muestra aleatoria en algunos casos es caro o difícil, incluso aunque dispongamos de la lista completa de la población. Imaginemos que quisiéramos estudiar la prevalencia y gravedad de la miopía de los estudiantes de Primaria de las Baleares.  Para ello, previo permiso de la autoridad competente, tendríamos que seleccionar una muestra representativa de los escolares de Baleares. Seguramente podríamos disponer de su lista completa y por lo tanto podríamos tomar una muestra aleatoria, pero entonces acceder a las niñas y niños que la formasen seguramente significaría  contactar con unos pocos alumnos de muchos centros de primaria, lo que volvería el proceso lento y costoso. Y eso si la *Conselleria d'Educació* nos facilitase la lista completa de alumnos.
...
A menudo una vez elegidos los *clusters* no se toman todos los sujetos de los mismos, sino una muestra aleatoria de cada uno. Esto ya será un ejemplo de muestreo **polietápico** (Sección \@ref(sec:poli)).
...
1. Se dividieron las Baleares en 14 sectores geográficos de entre 45.000 y 53.000 habitantes. Se ofreció la participación a los médicos de familia para conseguir que colaborasen 3-4 médicos por sector, hasta llegar a los 50 médicos.

2. De la población adscrita a cada médico participante se obtuvo una muestra aleatoria simple [de 64 individuos, 40 para alcanzar una muestra de 2000 sujetos y 24 de reserva para sustituir a los individuos no respondedores o no elegibles] de la que se comprobó la proporcionalidad a la edad y el sexo del sector.

Como véis, el muestreo tuvo dos etapas. La primera fue un muestreo no aleatorio de *clusters* estratificado: los estratos fueron los 14 sectores geográficos, los *clusters* elegidos eran los grupos de pacientes de los médicos que voluntariamente participaron en el estudio. La segunda fue un muestreo aleatorio simple dentro de cada *cluster*.


Otro ejemplo ago más complicad. En el estudio ["Factors associated with risk of low folate intake among adolescents"](https://www.scielo.br/pdf/jped/v82n2/en_v82n2a08.pdf) (M. Vitolo *et al*, *Jornal de Pediatria* 82 (2006), pp. 121–126) 
...
Un último ejemplo. Recordad el estudio sobre enfermedades y mes de nacimiento del Ejemplo \@ref(exm:quesada), donde "se escogió al azar una muestra de 29478 españoles". En  realidad, **no** se tomó una lista de toda la población española a 1 de enero de 2017 y se escogió al azar de manera equiprobable una conjunto de  29478 personas, sin que se usó la muestra de la *Encuesta Nacional de Salud* de ese año, que se escoge de la manera polietápica siguiente (podéis consultar [aquí](https://www.ine.es/metodologia/t15/t153041917.pdf) la metodología en detalle):
