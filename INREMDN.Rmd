...
* 2021-11-15: Subida la lección 14.
...
```{block2,type="rmdexercici"}
"Para estudiar si la vacunación reduce el riesgo de síntomas graves en la COVID-19, hemos tomado un grupo de infectados y hemos anotado por un lado la gravedad de sus síntomas y por otro lado si están vacunados, vacunados a medias o no vacunados." ¿Longitudinal o transversal?
```
...
Por lo tanto, tendríamos que hablar de "En los últimos 14 días se han detectado en las Islas Baleares 249 casos por cada cien mil habitantes **sin COVID-19**".
```
...
* Si escogemos 100 individuos de las Baleares (que tiene alrededor de 1,150,000 habitantes) al azar permitiendo repeticiones, la probabilidad de que se escoja más de una vez algún individuo es de `r round(pbirthday(100,1150000),3)`: solo en 1 de cada 250 muestras de 100 individuos de las Baleares elegidos al azar permitiendo repeticiones nos saldría alguien repetido.

* Si escogemos 100 estudiantes de la UIB (que tiene alrededor de 12000 estudiantes) al azar permitiendo repeticiones, la probabilidad de que se escoja más de una vez algún individuo es de `r round(pbirthday(100,12000),3)`: en  algo más de 1 de cada 3 muestras de 100 estudiantes de la UIB elegidos al azar permitiendo repeticiones habría alguien repetido. 
...
```{example}
Imaginad que queremos simular una secuencia de 100 lanzamientos de un dado cúbico equilibrado. El resultado sería una muestra aleatoria de tamaño 100 de valores entre 1 y 6,  con reposición porque (naturalmente) pueden repetirse valores. En vez de lanzar 100 veces el dado y anotar el resultado, podemos ejecutar
```
...
```{example, triola}
Un amigo cardiólogo nos contó una vez que la mitad de los enfermos que acudían a su consulta por supuestos problemas de corazón tenían en realidad estrés, y dos tercios, depresión. Nosotros vamos a suponer ahora que sólo un 15% de estos enfermos no sufrían ninguna de estas dos condiciones. 
...
```{block2,type="rmdnote"}
En realidad, la definición que hemos dado  de cuantil es "orientativa": no hay una regla única para calcular cuantiles de una muestra (*salvo la mediana*), y se han propuesto varios métodos que pueden dar  resultados diferentes; podéis consultar nueve de estos métodos en la  [entrada sobre cuantiles de la  Wikipedia en inglés](https://en.wikipedia.org/wiki/Quantile#Estimating_quantiles_from_a_sample). La razón de esta diversidad es que el objetivo final del cálculo de un cuantil puede no ser solo dar el primer valor cuya frecuencia relativa acumulada en el conjunto de datos llegue a $p$, sino también estimar qué vale este valor para el total de la población.
...
```{r cyanide2,echo=FALSE,out.width="80%", fig.cap="¿Con cuál te identificas?" }
...
```{block2,type="rmdimportant"}
Es una tontería, pero, por si acaso, queremos hacer hincapié en que si $X$ es $B(n,p)$, $P(X=k)$ no solo depende de $k$ sino también de los parámetros $n$ y $p$. Esto será general. Todas las variables aleatorias de una misma familia tienen la función de densidad de la misma forma, y solo varían los valores de los parámetros.
```
...
```{example}
Recordad uno de los ejemplos de variables no binomiales de la sección anterior. En una aula hay 5 chicos y 45 chicas. Escogemos 10 estudiantes, uno tras otro y sin repetirlos, para hacerles una pregunta. Cada elección es independiente de las otras. Contamos cuántos chicos hemos interrogado. Se trata de una variable hipergeométrica $H(5,45,10)$.
...
```{r,echo=FALSE,fig.width=10,out.width="90%"}
par(mfrow=c(2,2))
plot(0:10,dhyper(0:10,10,90,10),pch=20,cex=0.75,xlab="Número de éxitos",ylab="Probabilidad",type="h",main="H(10,90,10)")
plot(0:10,dhyper(0:10,30,70,10),pch=20,cex=0.75,xlab="Número de éxitos",ylab="Probabilidad",type="h",main="H(30,70,10)",col="red")
plot(0:10,dhyper(0:10,50,50,10),pch=20,cex=0.75,xlab="Número de éxitos",ylab="Probabilidad",type="h",main="H(50,50,10)",col="blue")
plot(0:10,dhyper(0:10,80,20,10),pch=20,cex=0.75,xlab="Número de éxitos",ylab="Probabilidad",type="h",main="H(80,20,10)",col="brown")
par(mfrow=c(1,1))
```
...
```{theorem}
Si $X$ es una variable aleatoria continua, la probabilidad de que tome cada valor concreto es 0:
$$
...
```{r epsilon, echo=FALSE, out.width="60%",fig.cap="El área bajo la curva alrededor de $a$ es aproximadamente igual a la del rectángulo de altura $f_X(a)$"}
...
```{block2,type="rmdnote"}
En realidad, que $f_X(x)$ sea $\geqslant 0$ para todo $x\in \mathbb{R}$ es consecuencia de que $F_X$ sea positiva y creciente (recordad que las funciones de distribución son siempre crecientes, porque si $x<y$, $F_X(x)=P(X\leqslant x)\leqslant P(X\leqslant y)=F_X(y)$) y coincida con $\int_{-\infty}^x f_X(x)\,dx$.
```
...
```{example,exhiperhipo}
La presión sistólica, medida en mm Hg, se distribuye como una variable normal con valor medio $\mu$ y  desviación típica $\sigma$ que dependen del sexo y la edad. Para la franja de edad 16-24 años, estos valores  son:
...
```{block2,type="rmdnote"}
Ahora que sabemos más cosas de la normal, en el Ejemplo \@ref(exm:exhiperhipo) nos hubiéramos podido ahorrar la mitad del trabajo. Llamemos $X$ a la variable aleatoria que nos da la presión arterial, en mm Hg, de un hombre de entre 16 y 24 años. Nos dicen que $X$ es $N(124,13.7)$. 
...
```{block2,type="rmdnote"}
Cuando se habla de un **intervalo de referencia** sin dar la probabilidad, se sobreentiende siempre que es el intervalo de referencia del 95%. Un 5% de la población cae fuera del intervalo de referencia del 95%.
```
...
Vamos a explicar de dónde sale esta fórmula, puesto que es un paradigma de cómo se obtienen la mayoría de las fórmulas paramétricas para intervalos de confianza. Quien se la quiera tomar como dogma de fe, que salte directamente al Ejemplo \@ref(exm:experimento).
...
donde `x` y `n` representan, respectivamente, el número de éxitos y el tamaño de la muestra, y `conf.level` es nuestra $q$, el nivel de confianza en tanto por uno.  El valor por defecto de  `conf.level` es 0.95, por lo que no hace falta especificarlo si queréis calcular un IC-95%.
...
$$
entonces seguro que $M\leqslant M_{max}$, valga lo que valga $\widehat{p}_{X}$.
