...
De todas formas, no hay una línea divisoria clara entre las series de casos y los *surveys*. Ni falta que hace.
```
...
```{block2,type="rmdcaution"}
Saber cuántas veces es mayor la fracción de expuestos entre los casos que entre  los controles, no nos permite saber en general cuántas veces es mayor la fracción de enfermos entre los expuestos que entre  los no expuestos.
```
...
```{block2,type="rmdcaution"}
El adjetivo "controlado" en la descripción de los estudios experimentales hace referencia a la existencia de al menos dos grupos que comparamos, uno de los cuales normalmente hace de "control". No a que el investigador en teoría "controle" todos los aspectos del estudio: eso está incluido en el adjetivo "intervencionista".
```
...
```{block2,type="rmdrecordau"}
Procurad tener siempre presente que, por mucho cuidado que pongamos en obtener una muestra de una población, numca será más que una aproximación imperfecta de esta.
```
...
```{example}
Para probar el caldo, antes de tomar una cucharada lo removemos bien. De este modo esperamos que las moléculas del caldo se organicen de manera aleatoria dentro de la olla y que la cucharada que tomemos sea representativa del caldo.
...
* Si escogemos 100 individuos de las Baleares (que tiene alrededor de 1,150,000 habitantes) al azar permitiendo repeticiones, la probabilidad de que se escoja más de una vez algún individuo es de `r round(pbirthday(100,1150000),3)`: solo en 1 de cada 250 muestras de 100 individuos de las Baleares elegidos al azar permitiendo repeticiones habría alguna repetición.
...
```{block2,type="rmdnote"}
Ya daremos el detalle de cómo se calculan todas estas probabilidades en el Tema \@ref(chap:probst).
```
...
```{r sesgosup, echo=FALSE,fig.cap="\"Sesgo de supervivencia\" (https://xkcd.com/1827/ (CC-BI-NC 2.5))",out.width="60%"}
...
* $P(B\cap A)$: Probabilidad de que un individuo de la población global pertenezca simultáneamente a $A$ y a $B$.
     ...
     Por ejemplo, con las notaciones anteriores, $P(A\cap B)$ es la probabilidad de que una persona   **sea mujer y lleve gafas**. La población en la que calculamos probabilidades vuelve a ser  la de todas las personas.
```
...
```{block2,type="rmdnote"}
Fijaos en que $P(A|B^c)\neq P(A|B)$, como anunciábamos que en general sucede.
```
...
```{block2,type="rmdcorbes"}
Se trata de ir operando astutamente:
$$
\begin{array}{l}
...
```{r idioma,echo=FALSE,fig.cap="Los aspirantes a médicos no os podéis perder la magnífica *Reglas médicas* en [Twitter](https://twitter.com/ReglasM) o [Facebook](https://www.facebook.com/reglasmedicas).",out.width="70%"}
...
```{r ROCama1,echo=FALSE,out.width="75%",fig.cap="Curva ROC de los niveles de PSA entre 1 y 15 como diagnóstico del cáncer de próstata en hombres blancos de 50 a 59 años.",fig.asp=1.5}
X=matrix(c(1,1,
           0.472 , 0.997 ,
 0.166 ,0.994,
0.068 ,0.978,
...
Lo de que aumente un 57.1% es incorrecto. Si el RA es 0.571, el riesgo de ST entre los expuestos  **no es un 57.1% mayor que entre los no expuestos, sino 57.1 puntos porcentuales mayor**.
```
...
```{r,echo=FALSE}
barplot(cumsum(prop.table(table(Hijos))),main="Frecuencias relativas acumuladas")
```
...
```{block2,type="rmdimportant"}
Es importante observar que, como ilustran los ejemplos y el chiste anteriores:
...
```{r salaris,echo=FALSE, out.width="75%",fig.cap="Gráfico publicado por el INE  (https://www.ine.es/prensa/eces_2018_a.pdf)"}
...
* El cuantil de orden 0.75, $Q_{0.75}$, es el primer elemento en esta lista ordenada que es  mayor o igual que, como mínimo, el 75% de los datos. Como el 75% de 20 es 15, $Q_{0.75}$ es el decimoquinto elemento de la lista ordenada: `r Valor[15]`.

* El cuantil de orden 1/3, $Q_{1/3}$, es el primer elemento en esta lista ordenada que es  mayor o igual que, como mínimo, un tercio de los datos. Como un tercio de 20 es 6.66 y pico, $Q_{1/3}$ es el *séptimo* elemento de la lista ordenada: `r Valor[7]`. Fijaos en que 6/20=0.3, y por lo tanto el sexto elemento de la lista deja a su izquierda solo el 30% de la muestra, no el 33.33...%. Necesitamos el séptimo elemento para llegar al tercio, aunque entonces nos pasemos. 
...
```{block2,type="rmdnote"}
En realidad, la definición que hemos dado  de cuantil es "orientativa": no hay una regla única para calcular cuantiles de una muestra (*salvo la mediana*), de manera que se han propuesto varios métodos que pueden dar  resultados diferentes; podéis consultar nueve de estos métodos en la  [entrada sobre cuantiles de la  Wikipedia en inglés](https://en.wikipedia.org/wiki/Quantile#Estimating_quantiles_from_a_sample). La razón de esta diversidad es que el objetivo final del cálculo de un cuantil puede no ser solo dar el primer valor cuya frecuencia relativa acumulada en el conjunto de datos sea mayor o igual que $p$, sino también estimar qué vale este valor para el total de la población.
...
```{block2,type="rmdromans"}
En realidad, en un histograma lo que representa la frecuencia de la clase es siempre el **área** de su barra. Pero si todas las clases tienen la misma amplitud, las áreas de las barras serán sus alturas por la amplitud común de las bases, y por lo tanto proporcionales a las alturas. En este caso, y **solo en este caso**, podemos interpretar que las alturas representan las frecuencias. Pero en la práctica, y por motivos que se entenderán al hablar de variables aleatorias en el próximo tema, esta representación de las frecuencias por medio de las alturas solo se lleva a cabo para frecuencias absolutas.
```
...
names(DF)=c("Clase","Frecuencia","Fre. acum.", "Frec. rel. acum.")
kable(DF)%>%
  kable_styling(full_width = F)
```
...
```{block2,type="rmdnote"}
Una nota técnica, para los completistas. ¿Qué pasa, al calcular los rangos, si hay valores repetidos?  Pues que se les asigna el mismo rango calculado de la manera siguiente: 
...
```{example,notes1}
Si, en una clase, un 10% de los estudiantes han sacado un 4 en un examen, un 20% un 6, un 50% un 8 y un 20% un 10, ¿cuál ha sido la nota media obtenida?
...
```{block2,type="rmdnote"}
Si existe algún $x_p\in D_X$ tal que $F_X(x_p)(=P(X\leq x_p))=p$, entonces el $p$-cuantil es ese $x_p$, porque, para todo otro $x\in D_x$:
...
```{block2,type="rmdimportant"}
Tenemos un subconjunto $A$ de una población $\Omega$ (por ejemplo, las personas que dan positivo en la PCR). Sea $p$ la proporción poblacional de personas que pertenecen a $A$, es decir $p=P(A)$. Tomamos **muestras aleatorias simples** de tamaño $n$ de la población y contamos cuántos sujetos de la muestra son de $A$. Esta variable aleatoria es **binomial** $B(n,p)$.
```
...
```{r, echo=FALSE, out.width="80%",fig.cap="La app *Probability Distributions*."}
...
```{block2,type="rmdrecordau"}
Fijaos en el orden de los argumentos de la función entre los paréntesis. Para calcular $f_X(x)$, aplicamos `dbinom`  a $(x,n,p)$.
```
...
```{block2,type="rmdmercifulgod"}
Os tenéis que saber la fórmula de Laplace, no hay que saber las fórmulas de los otros dos métodos. Pero sí cuándo se pueden usar y cuándo no.
...
```{block2,type="rmdnote"}
Esta fórmula es simplemente la traducción de la fórmula basada en la t de Student del IC-$q$, aplicada a estimar la media $\mu_1-\mu_2$ de la variable $X_1-X_2$ a partir de una muestra de valores de esta diferencia.
```
...
```{block2,type="rmdcaution"}
Si rechazamos $H_0$ en favor de $H_1$ **no** será porque hayamos demostrado que $H_0$ sea imposible, ni siquiera que sea improbable: tan solo habremos observado que es difícil de creer que sea verdad a la vista de los resultados de nuestro experimento.
```
...
```{example,juicio}
En un juicio (en el que el acusado es inocente si no se demuestra lo contrario, es decir, en el que estamos dispuestos a aceptar por defecto que es inocente), se busca evidencia de que el acusado es culpable. Por lo tanto, esta es la hipótesis alternativa. Así:
...
```{block2,type="rmdcaution"}
La pregunta (el contraste) os lo tenéis que plantear *a priori* a partir de hipótesis o suposiciones previas, antes de llevar a cabo la recolección de datos. No vale cambiar de contraste a la vista de los datos obtenidos.
```
...
```{example}
Supongamos que lanzamos la moneda 3 veces y obtenemos 3 caras. ¿Es evidencia suficiente de que está trucada?
...
```{example}
Supongamos que ahora lanzamos la moneda 10 veces y obtenemos 10 caras. ¿Es evidencia suficiente de que está trucada?
...
```{block2,type="rmdnote"}
Tenemos una hipótesis (la nula), realizamos un experimento para contrastarla y obtenemos un resultado que es muy improbable si la hipótesis de partida es verdadera. Una de dos:
...
```{r, echo=FALSE,fig.cap="\"Null hypothesis\" (https://xkcd.com/892/ (CC-BI-NC 2.5))"}
library(linguisticsdown)
...
```{block2,type="rmdimportant"}
Es costumbre tomar $\alpha=0.05$: algo menos que la probabilidad de sacar 4 caras seguidas con una moneda equilibrada.
```
