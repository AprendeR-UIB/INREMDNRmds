...
* Si decimos que un **intervalo de confianza del 95%** para la concentración media de una proteína en suero en individuos sanos medida en g/dl es [11,16], esto significa 
    ...
    * y que tenemos este 95% de seguridad porque hemos calculado este intervalo con una fórmula que da un intervalo que contiene la media poblacional un 95% de las veces que la aplicamos a muestras aleatorias del misma tamaño que la nuestra.
...
* Dibujamos los intervalos en un gráfico, donde aparecerán en gris los que aciertan y en rojo los que fallan. La recta vertical marca la media poblacional $\mu$.
...
Por lo tanto, estimamos con un 95% de confianza que el porcentaje medio de aumento de alcohol en sangre después de tomar 4 cañas de cerveza está entre el 40.5% y el 41.9%.
...
* La "probabilidad de equivocarnos" se reparte por igual a los dos lados del intervalo: en una fracción $q/2$ de las veces que aplicamos la fórmula, el valor real del parámetro estará a la izquierda del extremo inferior y en otra fracción $q/2$ de estas ocasiones estará a la derecha del extremo superior.
...
* $n\geq 100$.
* Tanto el número de éxitos como el número de fracasos en la muestra son $\geq 10$.
...
* Laplace, porque $n\geq 100$ y hay más de 10 éxitos y más de 10 fracasos.
...
Una típica pregunta al diseñar un estudio es ¿de qué tamaño he de tomar la muestra para garantizar que el margen de error en la estimación sea como máximo un valor concreto $M_{max}$?
...
```{example}
¿Cuál es el menor tamaño de una muestra que nos garantice un error de como máximo 0.05 al estimar una proporción $p_X$ usando un intervalo de confianza de Laplace del 95%?

```

Por el teorema anterior, para garantizar un error de 0.05 al calcular un IC-95% para una proporción $p_X$ usando la fórmula de Laplace, tenemos que usar una muestra de tamaño $n$ tal que
$$
...
Os damos las fórmulas, aunque no hace falta saberlas, solo recordar que la fórmula concreta a usar depende de estas condiciones. Supongamos, pues, que  $X_1$ y $X_2$ son aproximadamente normales o  que $n_1,n_2\geq 40$. En estas condiciones:
...
$$
donde, de nuevo, $\widetilde{S}_1^2$ y $\widetilde{S}_2^2$ son las varianzas muestrales de las muestras de $X_1$ y $X_2$, respectivamente, y
$$
...
+n_2}\cdot \Big(\frac{n_1 (1-\widehat{p}_1) +n_2( 1-\widehat{p}_2)}{n_1
