...
**En casos MUY sencillos**, esta proporción coincide con la fracción de veces en que se da este suceso en el conjunto de resultados posibles, es decir, 
$$
\text{Probabilidad}=\frac{\text{Casos favorables}}{\text{Casos posibles}}
$$

Pero esto solo es cierto cuando todos los resultados posibles tienen la misma probabilidad (y aquí entramos en un bucle: para poder aplicar esta fórmula hay que saber la probabilidad de cada uno de los resultados posibles, porque hay que garantizar que son iguales; pero no podemos calcularlas con esta fórmula sin saber que todas son iguales; pero para saberlo necesitamos calcularlas...)


Por ejemplo:

* La probabilidad de que salga cara al lanzar una moneda equilibrada al aire es 1/2 (casos favorables 1, casos posibles 2).

* La probabilidad de escoger al azar un estudiante varón en una clase de 50 mujeres y 10 hombres, si todos los estudiantes tienen la misma probabilidad de ser elegidos, es 10/60=1/6  (casos favorables 10, casos posibles 60).
...
* O más exageradamente, la probabilidad de que una mujer de entre 17 y 27 años sea miope no es 1/2, aunque solo haya dos resultados posibles: ser miope y no serlo. Esta probabilidad es la proporción de miopes en la población formada por todas las mujeres de es franja de edad (que, en España en 2018, se estimaba en un 65.4% según un [informe de la Asociación Visión y Vida](http://visionyvida.org/informe-prevalencia-de-la-miopia-en-los-jovenes-en-espana-2018/)).
...
Según [J. d'Alembert](https://es.wikipedia.org/wiki/Jean_le_Rond_d%27Alembert) es 2/3. Su argumento era el siguiente. Si en el primer lanzamiento sale cara, ya tenemos una cara. Si sale cruz, volvemos a lanzar otra vez y miramos si sale cara o cruz. Por lo tanto, hay solo 3 resultados posibles: cara en el primer lanzamiento; cruz en el primer lanzamiento y cara en el segundo; cruz en ambos lanzamientos. De estos tres "casos posibles", solo en los 2 primeros sale alguna cara. asos favorables partido por casos posibles: 2/3.

¿Qué falla en este argumento?
```


La teoría matemática de las probabilidades es la que nos permite estudiar lo exacta que es una estimación de un valor para una población a partir de una muestra. Por ejemplo, supongamos que elegimos al azar un grupo de 50 estudiantes de la UIB y observamos que 33 sufren de miopía. A partir de este dato, estimamos que un 66% de los estudiantes de la UIB son miopes. La técnicas de teoría de las probabilidades nos permitirán calcular la probabilidad de acertar con esta estimación con un margen de error fijado.
...
* **Tasa** de algo: Es equivalente a la "proporción" o la "fracción" de ese algo en algún total. Por ejemplo:

    * La **tasa de mortalidad** de una enfermedad es la proporción de individuos de una población que mueren a causa de esa enfermedad en un período determinado, y por tanto la probabilidad de que un individuo de esa población muera por esa enfermedad en ese período determinado de tiempo.
    
    * La **tasa de letalidad** de una enfermedad es la proporción de enfermos (de esa enfermedad, se entiende) en una población que mueren a causa de esa enfermedad en un período determinado, y por tanto la probabilidad de que un enfermo muera por esa enfermedad en ese período determinado de tiempo.
...
* La probabilidad de que un hombre sano cogiera cáncer de riñón en 1998 fue de 0.0001192 y en 2022 será de 0.000157.
...
* $A\cup B$ indica el conjunto de los estudiantes de la clase que son mujeres **o** llevan gafas.

* $A \cap B$ indica  el conjunto de los estudiantes de la clase que son mujeres **y** llevan gafas, es decir, las mujeres de la clase que llevan gafas.

* $B^c$  indica  el conjunto de los estudiantes de la clase que  **no** son mujeres, es decir el conjunto de los hombres  de la clase.

* $B^c$  indica  el conjunto de los estudiantes de la clase que  **no** llevan gafas.

* $A-B$  indica  el conjunto de las mujeres de la clase que **no** llevan gafas.

* $B-A$  indica  el conjunto de las hombres de la clase que **sí** llevan gafas.

* $A$ y $B$ serán incompatibles si no hay ninguna mujer en la clase que lleve gafas.
...
El hecho de que la unión y la intersección sean asociativas y conmutativas nos premite escribir expresiones del estilo de
$$
A_1\cap A_2\cap \cdots \cap A_n
$$
para indicar la intersección de los sucesos $A_1,\ldots,A_n$ agrupándolos como queramos y en el orden que queramos. Y lo mismo para la unión.
...
5. $P(A-B)=P(A)-P(A\cap B)$. 
    ...
    En efecto, como $A-B$ y $A\cap B$ son disjuntos (ningún elemento de $A-B$ pertenece a $B$ y todos los elementos de $A\cap B$ pertenecen a $B$) tenemos que $P((A-B)\cup (A\cap B))=P(A-B)+P(A\cap B)$; y por otro lado $(A-B)\cup (A\cap B)=A$, porque $A$ es la unión del conjunto de sus elementos que no pertenecen a $B$ y del conjunto de sus elementos que sí pertenecen a $B$ (Figura \@ref(fig:AmenysB)).
...
¡Ojo! En general, es falso que $P(A-B)=P(A)-P(B)$. Si en una población hay un 50% de mujeres y un 60% de miopes, y un 35% del total son mujeres miopes, las mujeres no miopes son un 15% del total de la población, no un -10%, ¿no? 
```
...
8. $P(A\cup B)=P(A)+P(B)-P(A \cap B)$.
    ...
    Como podéis ver en la Figura \@ref(fig:AmenysB), $A\cup B=(A-B)\cup (B-A)\cup (A\cap B)$ y estos tres conjuntos son disjuntos, por lo que podéis aplicar las propiedades 4 y 5:
$$
\begin{array}{rl}
...
\end{array}
$$
    Por ejemplo, si en una población hay un 50% de mujeres y un 60% de miopes, y un 35% del total son mujeres miopes, las personas que son mujeres o miopes (o ambas cosas, nuestra "o" nunca va a ser exclusiva si no lo decimos explícitamente)  forman un 75% de la población: al 50% de mujeres le sumamos el 25% de miopes que no son mujeres.
...
$$
(donde $|\ldots |$ denota el **cardinal** del conjunto entre las barras).

```{block2,type="rmdcorbes"}
Suponed que $\Omega$ tiene $n$ elementos, digamos $\Omega=\{x_1,\ldots,x_n\}$, y que $P(x_1)=\cdots=P(x_n)$. Sea $p$ esta probabilidad común a todos los elementos de $\Omega$. Entonces, por las propiedades 2 y 9,
$$
...
```{block2,type="rmdcaution"}
En general, es falso que $P(A\cap B)=P(A)\cdot P(B)$. Por ejemplo, en la problación con un 50% de mujeres, un 60% de miopes  y un 35% de mujeres miopes que hemos usado antes, 0.6·0.5 no es igual a 0.35.

Más adelante veremos cuándo esta igualdad sí se cumple.
```
...
1. Vamos a tomar una población de referencia de 10,000 donantes ¿Por qué este número? Veamos, fijaos en que la proporción de sujetos en $A\cap B$ es del 0.05%, es decir, 0.0005, y para facilitar los cálculos me gustaría que todos los números de sujetos en mi población fueran enteros, para no liarme con decimales. Como 0.0005·10000=5, 10,000 me vale.
...
En algunos contextos, las probabilidades se presentan en forma de **odds**  (la traducción más común en castellano es **momios**, pero se usan otras: posibilidades, oportunidades, ocasiones...). Las **odds** de un suceso $A$ son
$$
...
\end{array}
$$
Es decir, $A$ es más probable que $B$ si, y solo si, sus *odds* son mayores.
...
$$
Su valor es difícil de interpretar excepto su relación con 1:
...
\qquad =\dfrac{q\cdot\frac{ P(B)}{1-P(B)}}{1+q\cdot\frac{P(B)}{1-P(B)}}=\dfrac{q\cdot  P(B)}{1-P(B)+q\cdot P(B)}=\dfrac{q\cdot P(B)}{1+(q-1)P(B)}
\end{array}
$$
y por lo tanto la relación entre $P(A)$ y $P(B)$ no es lineal.
```
...
Como hay 33 estudiantes con gafas de un total de 50 estudiantes, esta probabilidad es  33/50=0.65


3. ¿Cuál es la probabilidad de que un estudiante sea mujer y lleve gafas? 

Como la probabilidad de ser mujer es 0.6 y la probabilidad de llevar gafas es 0.65, la probabilidad de ser mujer y llevar gafas es el producto: 0.6·0.65=0.39.
    
```{block2,type="rmderror"}
¿Qué os hemos dicho sobre que, en general, la probabilidad de la intersección NO es el producto de las probabilidades?
```
...
* $P(B)$: La probabilidad de que un individuo de la población global $\Omega$ satisfaga $B$.
     ...
     Por ejemplo, la probabilidad de que una persona escogida al azar lleve gafas

* $P(B|A)$: Probabilidad de que un individuo de la subpoblación definida por los que satisfacen $A$, satisfaga $B$.

     Por ejemplo, la probabilidad de que una **mujer**  escogida al azar lleve gafas

* $P(B\cap A)$: Probabilidad de que un individuo de la población global satisfaga simultáneamente $A$ y $B$.
      
     Por ejemplo, la probabilidad de que una persona   escogida al azar sea mujer y lleve gafas
```
...
\text{Estudio} & \text{Cien.} & \text{Der.} & \text{Econ.} & \text{Educ.} & \text{Ingen.} & \text{Letras} &  \text{Total}\\ \hline
...
En (a) nos piden la probabilidad de que, si un adulto cree ser hipertenso, sea hipertenso, es decir, la probabilidad de $A$ condicionada a $B$:
$$
P(A|B)=\frac{P(A\cap B)}{P(B)}=\dfrac{0.09}{0.25}=0.36
$$
Si un adulto cree ser hipertenso, hay un 36% de probabilidad de que lo sea.


En (b) nos piden la probabilidad de que, si un adulto  es hipertenso,  crea ser hipertenso, es decir, la probabilidad de $B$ condicionada a $A$:
$$
...
Esta vez vamos a usar primero el método de las frecuencias naturales. Vamos a suponer que tenemos una población de referencia de 100 enfermos que acuden al cardiólogo.
...
* Ya tenemos 15 pacientes sin estrés ni depresión,  35 con depresión pero sin estrés y 20 con estrés pero sin depresión, en total 70 pacientes. Los otros 30, del total de 100, tendrán ambas enfermedades.
...
Naturalmente, ser hipertenso y sacar cara son sucesos independientes. La probabilidad de que salga cara no tiene nada que ver con que el que la lance sea hipertenso. Por lo tanto
$$
...
Por lo tanto, son sucesos dependientes. De hecho, $P(B|A)=0.2/0.365=0.55> 0.33=P(B)$: la probabilidad de ser hipertenso entre los obesos es mayor que en global de  la población.
...
* $X$: Que un individuo dé positivo en el test; $P(X|A)=0.99$, $P(X|A^c)= 0.05$
...
1. Se calcula la tasa de mortalidad de esa enfermedad en una población cuya composición en franjas de edad fuera esa estándard, usando las tasas nacionales de mortalidad en cada franja calculadas en (1). 
...
* $\mathit{MC}$: Morir de la enfermedad en China
...
Tenemos que $P(\mathit{MC}|C_1)=0.001$, $P(\mathit{MC}|C_2)=0.003$ y $P(\mathit{MC}|C_3)=0.007$.

Por el teorema de la probabilidad total, la tasa de mortalida en China es
$$
\begin{array}{rl}
P(\mathit{MC})&=P(\mathit{MC}|C_1)\cdot P(C_1)+P(\mathit{MC}|C_2)\cdot P(C_2)+P(\mathit{MC}|C_3)\cdot P(C_3)\\
...
La tasa de mortalidad de esta enfermedad ajustada a la distribución de edades de la Unión Europea, $\mathit{TA}$, es:
$$
\begin{array}{rl}
\mathit{TA} & =P(\mathit{MA}|E_1)\cdot P(E_1)+P(\mathit{MA}|E_2)\cdot P(E_2)+P(\mathit{MA}|E_3)\cdot P(E_3)\\
& =0.001\cdot 0.115+0.003\cdot 0.654+0.007\cdot 0.191=`r round(0.001* 0.115+0.003* 0.654+0.007* 0.191,4)`
\end{array}
$$
Es decir, la tasa de mortalidad ajustada requerida es del `r 100*round(0.001* 0.115+0.003* 0.654+0.007* 0.191,4)`%, superior a la "real".
...
Volvamos a la pregunta de los dos ejemplos anteriores, pero para no hacer *spoilers*, vamos a resolverla con otro datos. Va, vamos a suponer que la probabilidad de dar positivo en la mamografía si se tiene cáncer es del 90%  y si no se tiene, del 10%, y que prevalencia del cáncer de mama es del 1%.
...
Más en general, si  $A_1,A_2,\ldots,A_n$ es una partición de $\Omega$ y $X$ un suceso tal que $P(X)>0$,
$$
...
Terminemos con un ejemplo donde aplicar la fórmula de Bayes es inútil.
...
Fijaos en que si llamamos $E$ al suceso "Tener la enfermedad" y $T$ al suceso "Dar positivo en el test", los datos que tenemos son $P(T)$, $P(T|E)$ y $P(T|E^c)$. Para arrancar con la fórmula de Bayes, necesitamos conocer $P(E)$, que es justamente la prevalencia de la enfermedad, que no sabemos.
...
$$
Un 76.6% de los positivos están enfermos.
