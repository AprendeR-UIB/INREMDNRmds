...
**En casos MUY sencillos**, esta proporción coincide con la fracción de veces en que se da este suceso en el conjunto de resultados posibles y por lo tanto se puede calcular con la **fórmula de Laplace**:
$$
...
* La probabilidad de que salga cara al lanzar una moneda equilibrada al aire es 1/2 (casos favorables, 1; casos posibles, 2; los dos resultados tienen la misma probabilidad porque suponemos que la moneda es equilibrada).

* La probabilidad de escoger al azar un estudiante varón en una clase de 50 mujeres y 10 hombres, si todos los estudiantes tienen la misma probabilidad de ser elegidos, es 10/60=1/6  (casos favorables, 10; casos posibles, 60; todos los estudiantes tienen la misma probabilidad de ser elegidos).
...
* $A-B=A \cap B^c$ es  la  **diferencia** "$A$ menos $B$": es el conjunto formado por los elementos de $A$ que no pertenecen a $B$.
...
10. Si $A=\{a_1,a_2,\ldots,a_k\}$, entonces
$$
P(A)=P(a_1)+P(a_2)+\cdots+P(a_k)
$$
(para simplificar, si $a\in \Omega$, escribimos $P(a)$ en vez de $P(\{a\})$). Esta propiedad  se deduce directamente de la propiedad (4).

11. Fijaos que de esta última propiedad se deduce la **fórmula de Laplace**  mencionada al principio de esta lección: Si $\Omega$ es finito y todos sus elementos  tienen la misma probabilidad, entonces 
$$
...
En efecto, suponed que $\Omega$ tiene $n$ elementos, digamos $\Omega=\{x_1,\ldots,x_n\}$, y que $P(x_1)=\cdots=P(x_n)$. Sea $p$ esta probabilidad común a todos los elementos de $\Omega$. Entonces, por las propiedades (2) y (10),
$$
...
```{block2,type="rmdcaution"}
¡Ojo! En general, es falso que $P(A\cap B)=P(A)\cdot P(B)$. Por ejemplo, en la población con un 50% de mujeres, un 60% de miopes  y un 35% de mujeres miopes que hemos usado antes, 0.6·0.5 no es igual a 0.35.
...
¿Cuál es la probabilidad de que un donante escogido al azar dé positivo en al menos uno de los dos tests? ¿Cuál es la probabilidad de que un donante escogido al azar dé positivo en VHS pero no en VIH?
...
1. Vamos a tomar una población de referencia de 10,000 donantes 
    ...
    ¿Por qué este número? Veamos, fijaos en que la proporción de sujetos en $A\cap B$ es del 0.05%, es decir, 0.0005, y para facilitar los cálculos me gustaría que todos los números que me salgan sean enteros, para no liarme con decimales. Como 0.0005·10000=5, parece que 10,000 me va a valer.
...
$$
y nos dicen cuántas veces es más probable $A$ que "no $A$".
...
```{block2,type="rmdnote"}
A veces, que unas *odds* valgan $a/b$ se expresadiciendo que son $a:b$, y se lee "$a$ a $b$". Por ejemplo, las *odds* de suspender anteriores son 2:3 o 2 a 3.
```
...
Como son un cociente de dos probabilidades, las *odds* de un suceso son siempre mayores o iguales que 0, incluyendo la posibilidad de valer  $\infty$, y pueden tomar cualquier valor entre 0 e $\infty$.
...
Como veremos en su momento, en los estudios de casos y controles, la manera correcta de presentar cuánto más probable es un desenlace si se ha estado expuesto a un factor de riesgo que si no, es por medio de una *odds ratio*. La  **odds ratio** (**razón de momios**, de oportunidades...) de dos sucesos $A$ y $B$ es el cociente de sus *odds*:
$$
...
```{block2,type="rmdromans"}
¿Pero no habíamos dicho que la *odds ratio* les había salido 1.7? Bueno, sí, hemos mentido. Lo que les da 1.7 es la "odds ratio ajustada", que no hemos definido y de la que con suerte hablaremos a final de curso.
```
...
Este valor la fracción  de los sujetos de $A$ que pertenecen a  $B$; es decir,  es la probabilidad de que si pasa $A$, entonces pase $B$.
...
* $P(A|B)$: Probabilidad de que un individuo de $B$ pertenezca a $A$.
     ...
     Por ejemplo, con las notaciones anteriores, $P(A|B)$ es la probabilidad de que una persona que lleva gafas sea mujer.
...
Comprobadlo en el Ejercicio anterior.
```
...
¿Cuál es la probabilidad de que si hacemos el test a un infectado, dé positivo? ¿Cuál es la probabilidad de que hagamos el test a un infectado y dé positivo?
...
```{example}
Una anécdota verídica. Un cardiólogo nos contó una vez que la mitad de los enfermos que acudían a su consulta por supuestos problemas de corazón sufrían de hecho estrés, y dos tercios, depresión. Nosotros vamos a suponer ahora que sólo un 15% de estos enfermos no sufrían ninguna de estas dos condiciones. 
...
```{example}
En los EEUU, un 33% de la población es hipertensa. Supongamos que escogemos al azar un estadounidense y le hacemos lanzar una moneda equilibrada al aire. ¿Cuál es la probabilidad de que sea hipertenso y saque cara?
...
```{theorem} 
Si $A$ y $X$ son dos sucesos y $P(A)\neq 0$,
$$
P(X)= P(X\cap A) +P(X\cap A^c)=P(A)\cdot P(X|A)+ P(A^c)\cdot P(X|A^c)
$$
```

Algunos de vosotros habréis usado en Bachillerato un árbol como el siguiente para representar este teorema:
...
1. Se calcula la tasa de mortalidad de esa enfermedad en una población cuya composición en franjas de edad fuera esa estándard y en la que la tasa de mortalidad en cada franja de edad fuera la de España. 

    De esta manera, se obtiene la tasa de mortalidad de esta enfermedad en una población  de composición la media europea, pero que en cada franja de edad tuviera la tasa de mortalidad española.
...
\end{array}
$$
Es decir, esta tasa de mortalidad ajustada  es del `r 100*round(0.001* 0.115+0.003* 0.654+0.007* 0.191,4)`%. Es un poco mayor que la "real" en China. Es razonable: vemos que la tasa de mortalidad crece con la edad, y la población china tiene una mayor proporción de niños y una menor proporción de ancianos que la europea.

```{block2,type="rmdexercici"}
Supongamos que en los Emiratos Árabes, donde hay 256 hombres por cada 100 mujeres ([CIA Woldfact](https://www.cia.gov/library/publications/the-world-factbook/geos/print_ae.html)), un 1% de los hombres y un 2% de las mujeres sufren de una determinada enfermedad.
...
A menudo nos encontramos con que conocemos las probabilidades condicionadas en una dirección pero las que nos interesa saber son precisamente las condicionadas en dirección contraria. Por ejemplo:
...
Volvamos a la pregunta de los dos ejemplos anteriores, pero para no hacer *spoilers*, vamos a resolverla con otro números Va, vamos a suponer que la probabilidad de dar positivo en la mamografía si se tiene cáncer es del 90%  y si no se tiene, del 10%, y que la prevalencia del cáncer de mama es del 1%.
...
**(1)** Supongamos que la probabilidad de que un individuo de una comunidad tenga una determinada enfermedad es de 0.23. ¿Cuáles de las afirmaciones siguientes son verdaderas?
...
**(2)** Si un 40% de los pacientes que tienen alguna enfermedad sobreviven un año y, de los supervivientes, un 30% muere antes de cinco años, ¿cuál es la probabilidad de que un paciente sobreviva a dicha enfermedad más de cinco años?
...
**(3)** Si nos dicen que en una determinada población infantil, un 36% han tenido la varicela, un 80%, paperas, y un 70%, el sarampión, ¿cuáles de las afirmaciones siguientes sobre esta población seguro que son falsas?
