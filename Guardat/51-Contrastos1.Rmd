...
En una muestra de 20 diabéticos de esta franja de edad, se obtuvo una concentración media de calcio en plasma de $\overline{x}=3.2$ mmol/l con una desviación típica muestral $\widetilde{s}=1.5$. Vamos a suponer que podemos considerar esta muestra de diabéticos jóvenes como aleatoria. 


Llamemos $X$ a la variable aleatoria "Tomamos un hombre diabético de 22 a 44 años y le medimos la concentración de calcio en plasma en mmol/l". Esta variable $X$ también sigue una ley normal, pero ahora no sabemos su media $\mu$ y queremos contrastar si es mayor que 2.5 o no.


Nuestra situación, pues, es un caso particular del caso general siguiente. Tenemos una variable aleatoria poblacional $X\sim N(\mu,\sigma)$ y planteamos el contraste
$$
\left\{\begin{array}{l}
H_{0}:\mu=\mu_0\\ 
H_{1}:\mu >\mu_0
...
Sea $T_0$ el valor que toma el estadístico de contraste $T$ en nuestra muestra. Rechazaremos $H_{0}$ si $T_0$ es más grande que cierto umbral $L_0$, que determinamos a partir de $\alpha$:
...
\right.
$$
Si $\alpha=0.05$ y $n=20$, el umbral a partir del cual rechazamos $H_0$ es $t_{n-1,1-\alpha}=t_{19,0.95}=$`qt(0.95,19)`=1.73.

En nuestra muestra tenemos que $\overline{x}=3.2$, $\widetilde{s}=1.5$ y $n=20$, por lo tanto el estadístico de contraste vale
$$
...
Como $2.09>1.73$, concluimos con un nivel de significación del 5% que el nivel medio de calcio en sangre en los jóvenes diabéticos es mayor que en los jóvenes sanos.
...
En nuestro ejemplo, ya hemos calculado $T_0=2.09$. Entonces,
$$
