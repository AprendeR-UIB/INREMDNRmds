...
Una **prueba diagnóstica** es cualquier tecnología que pueda servir para detectar un signo que se relacione con la patología de interés. Por ejemplo, un test de embarazo, un determinación del nivel de un marcador tumoral, una prueba PCR para el SARS-COV-2, etc.
...
* **Test diagnóstico**: determina la presencia o no de la enfermedad en individuos que muestran síntomas o que han dado positivo en un test de cribado.

Por ejemplo, distinguiríamos entre las *mamografías de cribado* como las que se realizan periódicamente a las mujeres a partir de una cierta edad, y las *mamografías diagnósticas* que se realizan a las mujeres que han detectado un bulto en un seno. 
 
Tanto en un caso como el otro, el objetivo de una prueba diagnóstica es diagnosticar una enfermedad. Por ahora vamos a considerar que nuestra prueba es **dicotómica**, en el sentido de que da solo dos resultados posibles: positivo o negativo. Lo deseable sería entonces que:
...
Por desgracia, casi nunca se tiene una prueba diagnóstica cien por cien segura en este sentido. Ni tan solo para detectar si un individuo está muestro o no, por lo que parece.
...
*(b)* La prevalencia de la enfermedad celíaca entre los niños de Uruguay, donde se realizó este estudio, es de 1/64. ¿Qué estimamos que valen el VPP y VPN de cada una de las dos pruebas de anticuerpos estudiadas para los niños uruguayos?

*(c)* La prevalencia de la enfermedad celíaca entre los niños de España es de 1/71. ¿Qué estimamos que valen el VPP y VPN  de cada una de las dos pruebas de anticuerpos estudiadas para los niños españoles?
...
Vamos a responder las preguntas para la prueba de anticuerpos antiendomisio, os dejamos la otra prueba como **ejercicio**.

Para nuestra prueba, tenemos la tabla de frecuencias siguiente
$$
\begin{array}{r|c|c|c}
& \text{Celíacos}\ E & \text{No celíacos}\ S & \text{Total} \\ \hline
\text{Positivos}\ + & 47 & 1 & 48  \\  \hline
\text{Negativos}\ - & 3 & 14 & 17 \\ \hline
\text{Total} & 50  & 15 & 65  
\end{array}
$$
...
* La **especificidad** es $P(-|S)= 14/15=0.93333$
...
*(b)* En primer lugar vamos a calcular los valores predictivos usando la fórmula de Bayes. Como $P(+|E)=0.94$, $P(-|S)=0.93333$ y en Uruguay $P(E)=1/64=0.015625$, por lo que $P(S)=0.984375$:

$$
\begin{array}{rl}
\text{VPP}\!\!\!\! & =P(E|+)=\dfrac{P(+|E)\cdot P(E)}{P(+|E)\cdot P(E)+P(+|S)\cdot P(S)}\\
&= \dfrac{0.94\cdot 0.015625}{0.94\cdot 0.015625+0.06667\cdot 0.984375}=0.1829\\[1ex]
\text{VPN} \!\!\!\!& =P(S|-)=\dfrac{P(-|S)\cdot P(S)}{P(-|S)\cdot P(S)+P(-|E)\cdot P(E)}\\
&= \dfrac{0.93333\cdot 0.984375}{0.93333\cdot 0.984375+0.06\cdot 0.015625}=0.999
\end{array}
$$

Es decir, 

* Un 18.29% de los niños uruguayos positivos en el test son celíacos.
* Un 99.9% de los niños uruguayos negativos en el test no son celíacos.

Vamos a repetir el cálculo usando frecuencias naturales. Para que nos salgan números redondos, vamos a partir de una población de referencia de 640,000 uruguayos.
...
\text{Positivos} & 9400  &  42000 &  51400 \\ \hline
\text{Negativos} & 600  & 588000 &  588600  \\ \hline
...
*(c)* ¿Y en España? Usando la fórmula de Bayes ahora con $P(E)=1/71=0.01408$, obtenemos
$$
\begin{array}{rl}
\text{VPP}\!\!\!\! & =P(E|+)=\dfrac{P(+|E)\cdot P(E)}{P(+|E)\cdot P(E)+P(+|S)\cdot P(S)}\\
&= \dfrac{0.94\cdot 0.01408}{0.94\cdot 0.01408+0.06667\cdot 0.98592}=0.1676\\[1ex]
\text{VPN}\!\!\!\! & =P(S|-)=\dfrac{P(-|S)\cdot P(S)}{P(-|S)\cdot P(S)+P(-|E)\cdot P(E)}\\
&= \dfrac{0.93333\cdot 0.98592}{0.93333\cdot 0.98592+0.06\cdot 0.01408}=0.9991
\end{array}
$$

* Un 16.76% de los niños españoles positivos en el test son celíacos.
* Un 99.91% de los niños españoles negativos en el test no son celíacos.


Con el método de las frecuencias naturales a partir de una población de referencia de 710,000 españoles, y razonando como antes, obtenemos: 
...
(La diferencia en el VPP es debida al redondeo.)
...
Al ser la prevalencia en España más pequeña, el VPP es más pequeño y el VPN más grande. Es lo razonable, porque a mayor prevalencia:

* Hay más enfermos, es más fácil encontrarlos, el VPP es mayor.
* Hay menos sanos, es más difícil encontrarlos, el VPN es menor.

```{block2,type="rmdcorbes"}
Si llamamos $x\in [0,1]$ a la prevalencia y fijamos los valores de $P(+|E),P(+|S),P(-|E),P(-|S)$, de manera que el VPP y el VPN sean una función solo de $x$, tenemos que
$$
\begin{array}{l}        
...
\geq 70\text{ años} & 0.380 & 0.968 & 0.070  \\ \hline \hline
\text{Asintomáticos} & 0.361 & 0.962 & 0.002 \\
\text{Sintomáticos} & 0.852 & 0.729 & 0.043 \\ \hline \hline
\text{Global} & 0.588 & 0.934 & 0.043  
\end{array}
$$
Las diferencias en la sensibilidad y especificidad entre las diferentes franjas de edad se pueden deber a las diferentes características físicas de los senos a diferentes edades.


Vamos a añadir algunos términos más sobre probabilidades para una prueba diagnóstica:

* Su **exactitud** es la probabilidad de acertar el diagnóstico. Es decir, $P(E\cap +)+P(S\cap -)$.
...
$$
     Un valor de $\text{LR}(+)$ por encima de 10 se toma como indicador de buen potencial diagnóstico. Por el contrario, un valor de $\text{LR}(+)$ cercano a 1 indica una prueba inútil ($+$ son $E$ sucesos aproximadamente independientes).
...
$$
    De nuevo, un valor de $\text{LR}(-)$ por encima de 10 se toma como indicador de buen potencial para descartar la enfermedad, y un valor de $\text{LR}(-)$ cercano a 1 indica una prueba inútil en este sentido.
...
Los cocientes de verosimilitud tienen la ventaja que transforman las *odds* de tener la prueba *a priori* (antes de realizar la prueba diagnóstica) en *odds* *a posteriori* (tras realizar la prueba diagnóstica). En concreto, sean:
...
```{example}
Se sabe que una mujer con hermano hemofílico tiene un 50% de probabilidades de ser portadora de un gen hemofílico. Una prueba genética para detectarlo basada en rDNA tiene una sensibilidad del 90% y una especificidad del 80%. Si una mujer con hermano hemofílico se hace la prueba y da negativo, ¿cómo modifica este resultado sus *odds* de ser portadora?
...
G. Stine, en el libro "AIDS Update 1999" (Prentice Hall, 1999), explica que en 1987 se notificó a 22 donantes de sangre de Florida que habían dado positivo en un test ELISA+Western Blot de VIH, que tiene sensibilidad y  especificidad ambas del 99.99%. De ellos, 7 se suicidaron. 
...
En las secciones anteriores hemos considerado solamente pruebas dicotómicas, que dan positivo o negativo sin ninguna gradación intermedia. Pero muchas veces una prueba diagnóstica da valores en un intervalo de números reales: concentraciones de algún metabolito, de algún  antígeno, de un marcador tumoral... En este caso, lo habitual es tomar un **valor de corte** y definir como **positivo** todo valor por encima (o por debajo, va a depender de la prueba concreta) de ese valor de corte y como **negativo** lo contrario. Por ejemplo, en el diagnóstico de la diabetes por medio del nivel de glucosa en sangre en ayunas, se toma como positivo un valor por encima de un cierto umbral. En cambio, en el diagnóstico de la anemia por medio del hematocrito, se toma como positivo un valor por debajo de un cierto umbral. 

De esta manera pasamos de una prueba diagnóstica que puede tomar todo un continuo de valores a una prueba dicotómica como las de las secciones anteriores, con su sensibilidad y su especificidad. Pero ¿cómo se ha llegado a definir el valor de corte que se usa para distinguir positivo de negativo? ¿Y cómo podemos aprovechar el hecho de que la prueba nos pueda dar todo un continuo de valores?


Para simplificar el lenguaje y fijar ideas, vamos a suponer que los enfermos obtienen un valor anormalmente alto en la prueba diagnóstica, y que por lo tanto queremos definir como **positivo** cualquier valor por encima de un umbral. La prueba puede tomar muchos valores reales, y para cada uno de ellos el test dicotómico asociado tendrá una especificidad y una sensibilidad.

La eficacia diagnóstica global de esta prueba diagnóstica se representa gráficamente por medio de una **curva ROC** (de *Receiver Operating Characteristic*. Esta curva se obtiene:

* Considerando varios valores posibles de la prueba, $x$.
* Para cada uno de estos $x$, considerar el test definido tomándolo como valor de corte (hemos quedado que, para fijar ideas, dará positivo en un paciente si el resultado de la prueba es mayor o igual que $x$ y negativo si es menor).
* Para el test definido por el umbral $x_0$, sean:

    * $+_{x_0}$: el suceso "Dar positivo en este test" (es decir, dar en la prueba un valor $\geq x_0$).
    * $-_{x_0}$: el suceso "Dar negativo en este test" (es decir, dar en la prueba un valor $< x_0$).
    * Especifidad${}_{x_0} = P(-_{x_0}|S)$: la especificidad de este test, es decir, la probabilidad de que un individuo sano de un valor $< x_0$.
    * Sensibilidad${}_{x_0} = P(+_{x_0}|E)$: la sensibilidad de este test, es decir, la probabilidad de que un individuo enfermo de un valor $\geq  x_0$.

* Para cada test asociado a un umbral $x$, definimos el punto del plano real de abscisa la tasa de falsos positivos del test definido por el umbral $x$ y ordenada su sensibilidad:
$$
...
Fijaos en que, en la situación que estamos considerando en la que definimos positivo cuando el valor obtenido es mayor que un umbral, si $x<y$ entonces el test con umbral $y$ da menos positivos que el test con umbral $x$. Por tanto, el punto $\large(P(+_y|S),P(+_y|E)\large)$ estará a la izquierda y por debajo del punto $\large(P(+_x|S),P(+_x|E)\large)$. Si hubiéramos definido positivo cuando el valor obtenido es *mayor**menor** que un umbral, entonces, cuando $x<y$, el test con umbral $y$ daría **más** positivos que el test con umbral $x$, por lo que, el punto $\large(P(+_y|S),P(+_y|E)\large)$ estaria a la derecha y por encima del punto $\large(P(+_x|S),P(+_x|E)\large)$.

```{block2,type="rmdimportant"}
La curva ROC representa la sensibilidad de la prueba, para diferentes umbrales, en función de su tasa de falsos positivos. No en función del umbral.
```


```{example}
Vamos a dibujar una curva ROC. En el estudio ["Age-specific reference ranges for serum prostate-specific antigen in black men"](https://www.nejm.org/doi/full/10.1056/NEJM199608013350502) (T. Morgan *et al*, *New England Journal of Medicine* 335 (1996), pp. 304-310) se da la tabla siguiente con la especificidad y la sensibilidad del diagnóstico del cáncer de prostata por medio del [nivel de PSA](https://medlineplus.gov/spanish/pruebas-de-laboratorio/prueba-de-psa-antigeno-prostatico-especifico/) en diferentes poblaciones de hombres estadounidenses, definidas por raza y franja de edad. En este caso, como vuestros padres seguro que saben, un nivel alto de PSA es señal de cáncer, y por lo tanto, en cada fila, se define como "positivo" el tener un nivel de PSA mayor o igual que el nivel de su primera columna.
...
La curva ROC se obtiene uniendo puntos de la forma (1-especificidad,sensibilidad). Vamos a calcular estos puntos para la subpoblación de hombres blancos entre 50 y 59 años, que es la que más nos preocupa.
...
La Figura \@ref(fig:ROCama1) muestra la curva ROC que corresponde a estos valores. En ella los puntos aparecen de izquierda a derecha en orden ascendiente en la tabla, es decir en orden descendiente del umbral de PSA. Para ayudar en su comprensión, al lado de cada punto hemos anotado el valor de PSA al que corresponde.
...
0.000 ,0.040,
0.000 , 0.031,
0.000 , 0.025),nrow=15,byrow=T)

plot(X,type="o",pch=20,xlab="1-especificidad",ylab="sensibilidad",
     xaxp=c(0,0.5,10),yaxp=c(0,1,10))
text(X[,1]+0.01,X[,2],col="red",cex=0.5,labels=15:1)
```

Una de las aplicaciones de la curva ROC es ayudar a decidir qué valor de corte tomar para definir el test diagnóstico dicotómico que se va a usar en la práctica.  Una estrategia muy común es tomar el **valor de corte óptimo**: el valor $x_o$ el que la exactitud de la prueba,
$$
P(+_{x_o}|E)+P(-_{x_o}|S)=\text{sensibilidad}_{x_o}+\text{especificidad}_{x_o}
$$
sea máximo. Recordemos que, para cada punto de la curva ROC
...
Por lo tanto, el punto que corresponda a la suma máxima de especifidad y sensibilidad  será el punto que dé la suma mínima de sus distancias a $x=0$ y a $y=1$. Es el punto más cercano al rincón superior izquierdo $(0,1)$ del cuadrado unidad que enmarca la curva. Por ejemplo, para la curva ROC de la Figura \@ref(fig:roc1), es el indicado con una flecha en el gráfico siguiente:

```{r roc2,echo=FALSE,fig.cap="Una curva ROC.",out.width="50%"}
...
El valor de corte óptimo sería entonces tomar un valor de PSA igual a 3.  En  la práctica, se toma como nivel de PSA a partir del cual uno se ha de preocupar en esa franja de edad el 4. ¿Por qué? Es un compromiso: en las pruebas de cribado se prima la especificidad, para poder descartar sanos,  y esta mejora del PSA 3 al PSA 4 manteniendo aún una sensibilidad del 75%, es decir, detectando un 75% de los enfermos. 

La curva ROC también se usa para evaluar la **capacidad discriminatoria global de la prueba**, es decir, la habilidad en general para distinguir sanos de enfermos para el continuo de umbrales posibles. Para empezar, para que la prueba diagnóstica sirva para detectar enfermos, para cada umbral la sensibilidad ha de ser mayor que la tasa de falsos positivos (ha de dar positivo con más frecuencia en sanos que en enfermos, ¿verdad?). Esto significa que la curva ROC ha de estar por encima de la diagonal principal $y=x$. Observad que los puntos de esta diagonal de esta diagonal principal corresponden a la situación "ordenada igual a abscisa", es decir, a $P(+|E)=P(+|S)$. Es decir, a que dar positivo y estar enfermo sean sucesos independientes, y que por lo tanto el test no sirva para nada. En el contexto de las curvas ROC, a la diagonal principal del cuadrado unidad se la llama  la **línea de no discriminación** por este motivo.
...
Como hemos comentado, un valor de la prueba es mejor cuanto más se acerca su punto de la curva ROC al extremo superior izquierdo del cuadrado unidad. Por lo tanto, cuanto más se se separe una curva ROC de la línea de no discriminación por su derecha y más se aproxime al ángulo que forman las rectas $x=0$ e $y=1$, mejor se entiende que es la prueba globalmente. Por ejemplo, en la Figura \@ref(fig:roc3), la curva ROC con puntos rellenos corresponde a una prueba diagnóstica con mayoy capacidad discriminatoria que la de los puntos vacíos.


```{r roc3,echo=FALSE,fig.cap="Una curva ROC.",out.width="50%"}
knitr::include_graphics("INREMDN_files/figure-html/roc3.png")
```


La **capacidad discriminatoria de la prueba** se mide con su **Área Bajo la Curva** (**AUC**,  en inglés). Este valor es, como su nombre indica, el área comprendida por la curva ROC, el eje de abscisas $y=0$ y la recta vertical $x=1$. Representa el **valor promedio de la sensibilidad** para todos los valores de corte de la prueba 

```{block2,type="rmdcorbes"}
Para los que visteis integrales en Matemáticas II del Bachillerato, recordad que esta área se obtiene integrando entre 0 y 1 la función que define la curva, y que la integral dividida por la longitud de la base, que es 1, es el valor promedio de la función sobre el intervalo. Por lo tanto, para ser precisos, la AUC es el **valor promedio de la sensibilidad como función de la tasa de falsos positivos**, pero tampoco importa hilar tan fino.
```
...
(R. Rodríguez *et al*, *Anales del Sistema Sanitario de Navarra* 39 (2016), pp. 115-122) se comparan dos técnicas de diagnóstico de la obstrucción intestinal: los niveles de la proteína C reactiva (PCR) y del enzima lactato deshidrogenasa (LDH) en plasma.

```

La Figura \@ref(fig:rocnavarra) muestra las curvas ROC de ambos tests, con sus valores de AUC y los valores de corte óptimos marcados. Vemos que el test basado en el nivel de LDH tiene globalmente una mayor capacidad discriminatoria que la otra, y además tomando como positivo a partir de los valores de corte óptimos en cada test, el nivel de LDH tiene especificidad y sensibilidad mayores.
...
**(2)** Estáis  comparando el diagnóstico clínico de úlcera gastroduodenal y su halllazgo en la autopsia en una serie de 10,000 pacientes. Al comenzar a analizar los datos se construye la siguiente tabla:
$$
\begin{array}{l}
...
finalmente os encuentráis en la siguiente tabla:
$$
\begin{array}{l}
...
A partir de ella y redondeando decimales, señalad
