...
Si $X\sim N(\mu,\sigma)$, entonces resulta que
$$
...
si $Z\sim N(0,1)$, $E(Z)=0$ y $\sigma(Z)=1$.
...
```{r,echo=FALSE}
curve(dnorm(x),xlim=c(-5,5),xlab="",ylab="",main="Densidad de N(0,1)",lwd=2,col="blue")
```


La distribución normal: 

* Es una distribución teórica, no encontraréis exacta en la vida real.
* Pese a su nombre, no es más "normal" que otras distribuciones continuas.
* Pero es muchas distribuciones de la vida real son aproximadamente nomales, porque:

> Toda variable aleatoria que consista en tomar $n$ de  medidas independientes de una o varias variables aleatorias y sumarlas tiene distribución aproximadamente normal **cuando $n$ es muy grande**, aunque las variables aleatorias de partida no sean normales


Por ejemplo, una variable binomial $B(n,p)$ se obtiene tomando $n$ medidas independientes de una variable Bernoulli $Be(p)$ y sumando los resultados. Por lo tanto, por la "regla" anterior, una  $B(n,p)$ tendría que ser aproximadamente normal si $n$ es grande. Pues sí, si $n$ es grande (pongamos mayor que 100, aunque si $p$ está lejos de 0 o 1 puede ser menor), la distribución de una variable $X\sim B(n,p)$ es aproximadamente la de una normal $N(np,\sqrt{np(1-p)})$, donde, recordad que si $X\sim B(n,p)$, entonces $\mu_X=np$ y $\sigma_X=\sqrt{np(1-p)}$. 
...
O podéis usar R o alguna aplicación para móvil o tablet. Par R, la normal es `norm`. Así, por ejemplo, si $X\sim N(1,2)$
...
Veamos algunas propiedades importantes de la densidad y la distribución de una variable normal.
...
```{r,echo=FALSE, out.width="60%"}
knitr::include_graphics("INREMDN_files/figure-html/simn.png")
```


* Recordad que $P(X\leq x)=F_X(x)$ es el área comprendida entre la densidad $y=f_X(x)$ y el eje de abscisas a la izquierda de $x$.
```{r,echo=FALSE, out.width="60%"}
knitr::include_graphics("INREMDN_files/figure-html/undernorm.png")
```
...
```{r,echo=FALSE, out.width="60%"}
knitr::include_graphics("INREMDN_files/figure-html/simnorm1.png")
```
...
$$
    Por lo tanto, $\mu$ es también la **mediana** de $X$.


* En particular, si $Z\sim N(0,1)$, las áreas a la izquierda de $-z$ y a la derecha de $z$ son iguales y la mediana de $Z$ es 0.
$$
...
```{r,echo=FALSE, out.width="60%"}
knitr::include_graphics("INREMDN_files/figure-html/simnorm2.png")
```


* Aumentar la $\mu$ desplaza a la derecha el máximo, y con él toda la curva. En el gráfico siguiente, $\mu_1<\mu_2$:

```{r,echo=FALSE, out.width="60%"}
knitr::include_graphics("INREMDN_files/figure-html/mu1mu2.png")
```

* Aumentar la $\sigma$ achata la curva: al aumentar la desviación típica, los valores se alejan más del valor medio. En el gráfico siguiente, $\sigma_1<\sigma_2$:
```{r,echo=FALSE, out.width="60%"}
knitr::include_graphics("INREMDN_files/figure-html/sigma1sigma2.png")
```

    El gráfico siguiente muestra el efecto combinado: $\mu_1<\mu_2$ y $\sigma_1<\sigma_2$:

```{r,echo=FALSE, out.width="60%"}
knitr::include_graphics("INREMDN_files/figure-html/musigma.png")
```



* Algunos números que tendríais que recordar:
...
1. Si $X\sim N(\mu,\sigma)$ y $a,b\in \mathbb{R}$, entonces
$aX+b$ es $N(a\mu+b,|a|\cdot\sigma)$

2. En particular, si $X\sim N(\mu,\sigma)$, entonces su **tipificada** (o **estandarizada**
$$
Z=\dfrac{X-\mu}{\sigma}
$$
es $N(0,1)$.
