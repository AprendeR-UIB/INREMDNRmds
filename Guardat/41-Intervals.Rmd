...
Los estimadores de la lección anterior nos permiten estimar el valor de una característica de una población, pero no nos indican el error que se comete con esta estimación.

A la práctica, el que se suele hacer es complementar una estimación puntual con un intervalo que mesure la precisión de la estimación. Esta precisión depende:

* De la variabilidad de la variable aleatoria de interés: $\sigma_X$

* De la medida de la muestra: $n$

* De la variabilidad del estimador (que seguramente depende de las dos anteriores): $\sigma_{\overline{X}}$, $\sigma_{\widehat{p}_X}$...

* Del nivel de confianza*, o seguridad*, de la estimación: como de seguros queremos estar que la estimación es correcta


```{r, echo=FALSE, fig.width=5}
knitr::include_graphics("INREMDN_files/figuro-html/Garfield.png")
```

## Definiciones básicas

Un intervalo de confianza del Q%* (abreviadament, un *IC Q%) de un parámetro poblacional es un intervalo obtenido aplicando a una muestra aleatoria simple de medida $n$ una fórmula que satisface la propiedad siguiente: 

```{block2, type='rmdimportant'}
El intervalo obtenido contiene el valor del parámetro poblacional el Q% de las veces que lo aplicamos sobre muestras aleatorias simples de medida $n$ tomadas al azar
```

Tener una confianza del Q%* significa pues que empleamos una fórmula que acierta el Q% de las veces*; o, para ser precisos, el Q% de las veces que lo aplicamos bien*. Pero asumimos que uno (100-Q)% de las veces que lo aplicamos nos equivocamos, y no sabemos cuál es nuestro caso*.


```{example}
En un experimento hemos mesurado el porcentaje de aumento de alcohol en sangre a 40 personas después de tomar 4 cañas de cerveza. La media y la desviación típica muestral de estos porcentajes de incremento han estado $\overline{x}=41.2$ y $\widetilde{s}=2.1$. Cómo veremos al Ejemplo \@ref(exm:alcohol1), un IC 95% para el porcentaje de aumento mediano de alcohol en sangre de una persona después de beber 4 cañas de cerveza es [40.53, 41.87].

Esto significa que estambre seguros al 95%* que el aumento mediano de alcohol en sangre de una persona después de beber 4 cañas de cerveza está entre el 40.53% y el 41.87%, porque este intervalo lo habremos calculado con una fórmula que el 95% de las veces que lo aplicamos (bien) sobre muestras aleatorias de 40 personas da un intervalo que contiene la media poblacional que queremos estimar. Nosotros somos optimistas y "confiam" estar dentro de este 95% de aciertos.
...
* Intervalo de confianza del Q% para un parámetro*: Intervalo que contiene el valor poblacional del parámetro de la variable aleatoria* "con probabilidad" Q%, en el sentido que lo hemos calculado con una fórmula que da un intervalo que contiene el parámetro el Q% de las veces que lo aplicamos a una muestra aleatoria.
...
* Si decimos que un intervalo de referencia del 95%* para la concentración de una proteína en suero en individuos sanos mesurada en g/dl es [11,16], esto significa significa que un 95% de los individuos sanos tienen una concentración de esta proteína en suero entre 11 y 16 g/dl, o, equivalentemente, que un individuo sano escogido al azar tiene, con un 95% de probabilidad, una concentración de esta proteína en suero entre 11 y 16 g/dl

* Si decimos que un intervalo de confianza del 95%* para la concentración mediana de una proteína en suero en individuos sanos mesurada en g/dl es [11,16], esto significa que hemos tomado una muestra aleatoria de concentraciones de esta proteína en suero en individuos sanos y a partir de esta muestra hemos estimado que, con un 95% de confianza, la concentración mediana de esta proteína en suero en individuos sanos está entre 11 y 16 g/dl (y tenemos un 95% de confianza en este intervalo porque lo hemos calculado con una fórmula que da un intervalo que contiene la media poblacional un 95% de las veces que lo empleamos sobre muestras aleatorias de la misma medida que la nuestra).

* Si decimos que el 95% de las muestras de 100 concentraciones de una determinada proteína en suero en individuos sanos tienen la media muestral entre 11 y 16, esto es un intervalo de referencia del 95% para la media muestral* de muestras de medida 100, no un intervalo de confianza para la concentración mediana poblacional ni un intervalo de referencia para el valor de la concentración en un individuo.


Que un IC Q% para un parámetro $\theta$ sea $[a,b]$ sirve:

* Para estimar $\theta$ con este margen de confianza*: Estambre bastante seguros que el valor poblacional de $\theta$ está entre $a$ y $b$ (la fórmula empleada acierta a menudo)

* Para poder comparar el valor poblacional de $\theta$ con un valor concreto con este margen de confianza*: Estambre bastante seguros que el valor real de $\theta$ no está ni por debajo de $a$ ni por encima de $b$ y por tanto que es diferente de todos estos valores

Por ejemplo:* si un IC 95% para la *prevalència** $p$ de una determinada característica en una población (la fracción de individuos que tienen esta característica) va de 0.025 a 0.047:

* Estambre mucho ("un 95%") seguros que $p\in [0.025,0.047]$ (porque la fórmula empleada para calcular este intervalo acierta en un 95% de las veces)

* Estambre muy seguros que $p>0.02$ (porque todo el intervalo donde estambre muy seguros que cae el valor real de $p$ está a la derecha de 0.02)

* Estambre muy seguros que $p
eq 0.05$ (porque 0.05 no pertenece al intervalo donde estambre muy seguros que cae el valor real de $p$)

* Pero no estambre muy seguros* que $p=0.03$, por mucho que $0.03\in [0.025,0.047]$: estambre muy seguros que $p$ está entre 0.025 y 0.047, pero no tenemos ninguna seguridad que valga un valor concreto entre estos límites, con solo que está entre estos límites.


Hay dos tipos de métodos básicos de cálculo de intervalos de confianza a partir de una muestra aleatoria:


* Paramétricos**: Usando alguna fórmula basada en la distribución muestral del estimador

* Se basan en teoremas

* Solo sirven si la variable aleatoria $X$ y la muestra aleatoria satisfacen (aproximadamente) las hipótesis del teorema

* No paramétricos*: El más empleado es lo *bootstrap**:

* De la muestra, se toman al azar muchas (miles) muestras aleatorias simples de la misma medida que la muestra, se calcula el estimador con cada una de estas muestras y se usa el vector de resultados para estimar un intervalo de confianza (por ejemplo, podríamos tomar como IC 95% el intervalo entre los cuantiles 0.025 y 0.975 de este vector)

* Se puede usar siempre (si la muestra es aleatoria)

* Emplea un proceso aleatorio: en cada ejecución sobre los mismos datos puede dar un intervalo diferente

```{block2,type="rmdnote"}
Lo bootstrap* es una herramienta muy poderosa para calcular intervalos de confianza y, en general, para estimar la distribución muestral de un estadístico. Tanto, que a la práctica ya empieza a sustituir los métodos paramétricos. Pero no hace milagros: si la muestra es pequeña o muy poco representativa de la población, un IC calculado con el bootstrap servicio de tan poco como uno de calculado con un método paramétrico.
```
...
Prenem 5000 mostres aleatòries simples de mida 25 (la mateixa mida que el conjunt de dades original) de les dades i calculam la mitjana de cada mostra; fixam la llavor d'aleatorietat per que el càlcul sigui reproduïble:
```{r}
set.seed(100)
...
Per exemple, l'IC 90% per al nombre mitjà de radiografies per cartutxo de l'Exemple \@ref(exm:RXprint) es calcularia amb
```{r}
...
Per fer-ho, prenem una mostra aleatòria simple de $X$ de mida $n$, amb nombre d'èxits $S$ i per tant proporció mostral  d'èxits $\widehat{p}_{X}=S/n$
...
```{theorem}
Un interval de confiança de nivell de confiança $q$ per a $p_X$ és $[p_0,p_1]$, on (recordau que $n$ indica la mida de la mostra i $S$ el nombre d'èxits)
...
D'entrada cal dir que si podem calcular més d'un interval, segurament donaran molt parescuts, com heu pogut comprovar a l'exemple anterior. A més, recordau que les tres fórmules només ens donen "un nivell de confiança q" si s'apliquen a mostres aleatòries simples, i les nostres mostres gairebé sempre seran oportunistes, cas en el qual, si ens posam perepunyetes, no en podem aplicar cap.
...
\displaystyle P(\chi_{n-1}^2\geq \chi_{n-1,(1+q)/2}^2)=1-\frac{1+q}{2}=\frac{1-q}{2},
\end{array}
$$
tenim que
$$
...
```{block2,type="rmdcaution"}
Fixau-vos que aquest interval de confiança per $\sigma^2$ no està centrat en $\widetilde{S}_{X}^2$ ni en ${S}_{X}^2$. A més, com que  $\chi_{n-1}^2$ no és simètrica, s'han de calcular els dos quantils  $\chi_{n-1,(1-q)/2}^2$ i $\chi_{n-1,(1+q)/2}^2$, perquè no hi ha cap relació entre ells que doni el valor d'un d'ells a partir del de l'altre.
```
