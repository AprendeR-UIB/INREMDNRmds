...
* Los **cuartiles** son los cuantiles $Q_{0.25}$, $Q_{0.5}$ y $Q_{0.75}$, y reciben, respectivamente, los nombres de **primer cuartil**, **segundo cuartil** (o mediana) y **tercer cuartil**. $Q_{0.25}$ será, pues, el menor valor que es mayor o igual que  un  cuarto de los datos, y $Q_{0.75}$, el menor valor que es mayor o igual que   tres cuartos de los datos.
...
Un conjunto de datos platicúrtico tiene las colas más altas de lo que cabría esperar si se ajustara a una campana de Gauss, mientras que si es leptocúrtico sus colas son más bajas.
...
La varianza y la desviación típica que calcula son las muestrales y el *RIC* es el rango intercuartílico.
...
Sean  $[L_c, L_{c+1})$ este intervalo crítico, $N_{c-1}$, la frecuencia absoluta acumulada del intervalo anterior al crítico (si el intervalo crítico es el primero, tomamos $N_{c-1}=0$), y $N_c$, la frecuencia absoluta acumulada del intervalo crítico. Entonces, lo que hacemos es unir con una recta los puntos $(L_c,N_{c-1})$ y $(L_{c+1},N_c)$ y aproximar  la mediana por medio de la abscisa $M$ del punto sobre esta recta cuya ordenada es $n/2$ (véase la Figura \@ref(fig:medianagrup)). La fórmula concreta  para $M$, por si no os acordáis de la ecuación de la recta determinada por dos puntos, es
$$
...
Por lo tanto, si los puntos están sobre una recta horizontal o una recta vertical, la correlación es 0.
```
