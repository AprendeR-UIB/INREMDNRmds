...
El adjetivo **significativo**, que ha salido dos veces y hemos entrecomillado en el párrafo anterior, tiene un significado muy concreto en estadística, sobre el que volveremos más adelante en el curso. Por ahora lo tomaremos en el sentido intuitivo de **tan grande que nos hace sospechar que también hay diferencia en la población en general**. Por ejemplo, la diferencia entre el 52% de fumadores entre los casos y el 30%  de fumadores entre los casos fue tan grande, que sería muy improbable si las proporciones de fumadores entre casos y controles en la población general fueran iguales.
...
* Se puede dar el mismo problema con los **confundidores** que en los estudios de casos y controles.
     ...
     Por ejemplo, en el estudio de la mortalidad y la psoriasis del Ejemplo \@ref(exm:psoriasis), está claro que la causa del aumento de mortalidad  no es  la psoriasis, sino enfermedades asociadas a ella.
...
En un **estudio transversal** se toma un grupo de individuos en principio representativo de una población (una **muestra transversal** de la población) y se miden simultáneamente, o  en un corto espacio de tiempo, varias características de todos ellos: por ejemplo, una enfermedad y los factores de riesgo relacionados con la misma y presentes en ese momento. El objetivo es inferir qué asociación hay entre estas condiciones  en la población general.
...
Y seguramente también podríamos considerarlo de cohorte retrospectivo, aunque para clasificarlo como tal es necesario que las enfermedades fueran tales que los sujetos no las tuvieran en el momento de nacer (lo fueron). Recordad que en los estudios de cohorte, en el momento en el que se clasifican los sujetos según su exposición a factores de riesgo han de estar **sanos**.
...
* **Prevalencia de período**: número (o proporción) de **personas** que en algún momento de un período de tiempo concreto  han tenido la enfermedad, incluidos los que ya estaban enfermos al inicio del período de tiempo considerado. Si una persona la ha tenido varias veces, solo cuenta una vez en este valor. 

* **Prevalencia de vida**: número (o proporción) de **personas** que en algún momento de su vida han tenido la enfermedad. De nuevo, si una persona la ha sufrido varias veces a lo largo de su vida, solo cuenta una vez en este valor.
...
*(b)* Teniendo en cuenta que la tuberculosis era una de las causas principales de hospitalización en el hospital donde se llevaron a cabo las autopsias, ¿creéis que su conclusión estaba justificada?
...
¿Para qué sirve entonces un estudio ecológico? Básicamente, para aportar evidencia indirecta de la veracidad de una hipótesis que puede reforzar otras evidencias o motivar estudios posteriores. Como además los estudios ecológicos son muy fáciles de llevar a cabo, ya que en general los datos en los que se basan son públicos en bases de datos al alcance de todos y se recogen en una tarde, son bastante comunes en epidemiología.
...
```{block2,type="rmdcaution"}
El adjetivo "controlado" en la descripción de los estudios experimentales hace referencia a la existencia de al menos dos grupos que comparamos, uno de los cuales normalmente hace de "control". No a que el investigador "controle" todos los aspectos del estudio: eso está incluido en el adjetivo "intervencionista".
```
...
El resto de criterios son optativos, pero cuantos más se cumplan, más fuerza tiene el argumento en favor de la relación causa/efecto.  Vamos a ilustrarlos con los argumentos en favor de que niveles altos de colesterol en sangre aumentan el riesgo de enfermedad coronaria.
...
Al buscar información sobre un tema médico específico, es importante formular la pregunta de manera adecuada. Un esquema muy usado para estructurar este tipo de preguntas es el **PICO**, o su variante **PIO** , acrónimo de **P**aciente-**I**ntervención-**C**omparación-**O***utcome* (resultado).
...
* ¿Qué resultado (**O***utcome*) nos interesa? Aquí **resultado** refiere a las consecuencias de la intervención referidas a la salud del paciente.
...
* ... ¿aumenta la supervivencia a 30 días vista?
