...
Un **estudio** es un proceso cuyo objetivo es obtener evidencia empírica sobre alguna cuestión. En el caso de los estudios **médicos** que nos ocupan en este curso, esta cuestión trata, naturalmente, sobre algún aspecto de la salud de las personas:  la efectividad de un tratamiento de una determinada enfermedad, la precisión de un test diagnóstico, las causas de una enfermedad, algún tema de salud pública... 

## Pasos de un estudio médico {#sec:pasos}

Para ir calentando motores, vamos a describir brevemente la estructura habitual de un estudio médico, y lo ilustraremos con el artículo ["Marcha nórdica para prevención cardiovascular en pacientes con cardiopatía isquémica crónica o síndrome metabólico"](https://www.sciencedirect.com/science/article/pii/S0025775316305024) (C. Vehí *et al*,^[Este *et al* significa "y otros" y se utiliza para no dar toda la lista de autores de un artículo.] *Medicina Clínica* 147 (2016), pp. 537-539) que convendría que fuerais consultando en paralelo con esta sección.

En un estudio médico por norma general se llevan a cabo los pasos siguientes:

1. Se deinen los **objetivos** de la investigación. En este primer paso,
    ...
    * El objetivo específico es estudiar si la marcha nórdica reduce los factores de riesgo cardio-vascular en los individuos del área sanitaria mediterránea.
...
* El **Resumen** (*Abstract*) al principio, describiendo lo más relevante de cada una de las cuatro partes centrales del artículo. Este resumen está relacionado con el último paso del proceso de investigación, la publicación de resultados, ya que es lo único que leemos en la mayoría de los casos, entrando solo en el detalle del artículo cuando nos interesa de verdad.
...
* **Serie de casos**: Descripción detallada de un conjunto pequeño de pacientes (pero más de uno, o sería un informe de *un* caso) con alguna característica en común que hace que tenga sentido agruparlos; la información se recoge por medio de los historiales clínicos  y las entrevistas clínicas.
...
```{example, diabetesSC}
El artículo ["Diabetes tipo 2 en niños. Serie de casos"](http://www.scielo.org.pe/pdf/rmh/v26n1/a02v26n1.pdf)  (H. Manrique-Hurtado *et al*, *Revista Médica Herediana* 26 (2015), pp. 5-9) es, como indica su título, un ejemplo típico de serie de casos. Se trata de una descripción detallada de las características de todos los niños y adolescentes diagnosticados de diabetes tipo 2 (DM2) en dos hospitales de Lima (Perú) entre 2008 y 2013. En total, se describen 32 sujetos de entre 8 y 19 años.

Entre otras características, observaron que, en el momento del diagnóstico el 85% de ellos (27 sujetos) presentaban acantosis pigmentaria (un transtorno de la piel); que un 17% (5 sujetos) tenían  cetoacidosis diabética (CAD), que suele ir asociada a diabetes  tipo 1 y no 2; y que estos últimos presentaban todos una serie de características comunes, por ejemplo todos tenían acantosis pigmentaria. 
...
```{r,doll, echo=FALSE,fig.cap="Consumo de tabaco en la muestra analizada por R. Doll y R. Bradford Hill en \"Study of the Aetiology of Carcinoma of the Lung\", *British Medical Journal* 2 (1952), pp. 1271–1286."}
...
En este estudio, los números de casos y controles no se prefijaron en su diseño, sino que se tomaron "los que encontraron" en un grupo de individuos. Diremos entonces que el grupo de sujetos analizados forman una muestra **transversal**.
...
Volvamos al Ejemplo \@ref(exm:surveycovid). Una vez identificados los 24 cirujanos que habían tenido la COVID-19, se escogieron al azar 48 cirujanos ortopédicos (dos por cada enfermo) de entre los que habían respondido el cuestionario que **no** habían tenido (aún) la COVID-19. Se compararon entonces los datos sobre la exposición previa a algunos factores de riesgo entre ambos grupos: grado de fatiga, horas de sueño, uso de mascarilla al tratar con pacientes, etc. Esta segunda parte del estudio es claramente de **casos** (cirujanos que habían tenido COVID-19) y **controles** (cirujanos que no habían tenido COVID-19).


```

En este estudio chino se puede observar la gran ventaja de un estudio de casos y controles sobre un estudio descriptivo: el disponer del grupo de control para poder comparar. Por ejemplo, un 58.7% de los cirujanos que habían tenido COVID-19 no habían recibido formación específica reciente sobre control de infecciones. Son muchos, pero esta cifra aislada no sirve para concluir que no haber recibido formación específica reciente sobre control de infecciones aumenta el riesgo de contraer la COVID-19. A lo mejor ese 58.7% va y es el porcentaje global de cirujanos ortopédicos de Wuhan que no habían recibido este tipo de formación, en cuyo caso no habría diferencia entre haber tenido o no COVID-19 en este aspecto.


El grupo de control es el que nos permite comparar este porcentaje con la población "sana". El porcentaje de controles que no habían recibido formación específica reciente sobre control de infecciones fue solo del 33%. Ahora sí vemos que la proporción de "expuestos al factor de riesgo" (no recibir este tipo de formación) entre los casos  es mucho mayor que entre los controles.


```{block2,type="rmdnote"}
Al problema de disponer de la proporción de expuestos solo en el grupo de enfermos, sin conocer esta proporción en un grupo de control (sea de sanos, sea una muestra transversal representativa de la población) se le suele llamar **problema de falta de denominador**. Cuando más adelante hablemos de riesgos relativos (Sección \@ref(sec:probaplic2)) se entenderá qué hace aquí la palabra "denominador".
```
...
* Podemos querer estudiar si ser fumador aumenta la probabilidad de fracaso escolar: tomamos como casos un grupo de personas que no llegaran a terminar la ESO y como controles un grupo con estudios universitarios, y les pedimos si durante sus estudios de secundaria fumaban y cuánto.
...
* Son adecuados para estudiar enfermedades raras, poco frecuentes o de desarrollo muy corto, ya que podemos partir de un grupo de enfermos y un grupo de sanos sin tener necesidad de encontrar enfermos en una muestra transversal de individuos.
...
Si no se va con cuidado al planearlos o al analizar los resultados, los estudios de casos y controles (y en general todos los estudios observacionales) son muy susceptibles al efecto de confundidores. Un **confundidor** es una característica asociada con la exposición que puede causar la enfermedad. 
...
No, ¡qué va! Más adelante, en el artículo ["Maternal smoking and Down syndrome: the confounding effect of maternal age"](https://academic.oup.com/aje/article/149/5/442/135974) (C. L Chen *et al*,  *American Journal of Epidemiology* 149 (1999), pp. 442-446) se cayó en la cuenta de que no se había tenido en cuenta la edad de las madres. Las madres jóvenes suelen ser fumadoras con mayor frecuencia que las no tan jóvenes, y las madres de más de 40 años tienen una mayor frecuencia de hijos con síndrome de Down. Aquí la edad es el factor confundidor que hace que parezca que las mujeres fumadoras tienen menor probabilidad de tener hijos con síndrome de Down, cuando en realidad son las madres jóvenes las que son más fumadores y tienen menos hijos con sindrome de Down.

```{r down2,echo=FALSE, out.width="50%"}
knitr::include_graphics("INREMDN_files/figure-html/Down2.png")
```

Seguimos con los inconvenientes.  Los estudios de casos y controles son muy susceptibles a algunos tipos de  sesgos concretos relacionados con la manera cómo se recogen los datos. Un **sesgo** en un estudio es un error sistemático que afecte a los datos recogidos y pueda perjudicar la corrección de las conclusiones obtenidas. Estudiaremos el tema de los sesgos en general con más detalle en la Sección \@ref(sec:sesgos). Por ahora, vamos a anotar varios sesgos típicos de los estudios de casos y controles.

* Si los datos sobre exposición a riesgos se recogen mediante entrevistas o cuestionarios, los sujetos pueden mentir (recordad la máxima "Todo el mundo miente" del Dr. G. House), consciente (para ocultar comportamientos de los que se avergüencen, para contentar el entrevistador,...) o inconscientemente (por haber olvidado detalles relevantes). O al menos puede que no recuerden bien los detalles. En ambos casos diremos que se produce un **sesgo de recuerdo**.
    
```{block2,type="rmdromans"}
Hay [un estudio  famoso](https://academic.oup.com/jnci/article-abstract/21/4/713/869173) en el que se preguntó a un grupo de hombres si estaban circuncidados: un 34% de los que contestaron que sí, en realidad no lo estaban, y un 34.6% de los que contestaron que no, en realidad sí que lo estaban.
```
...
Así por ejemplo, en el estudio sobre alcoholismo y cáncer de pulmón, el hecho de escoger los controles entre individuos hospitalizados podría haber sesgado este grupo, ya que entre los hospitalizados podría ser más frecuente el alcoholismo (ya que la ingesta desmesurada de alcohol aumenta el riesgo de patologías que no sean cáncer de pulmón pero que impliquen ingreso hospitalario) que en la población en general, por lo que tuvieron que tener mucho cuidado en este aspecto.

O en el ejemplo sobre el fracaso del tratamiento de la tuberculosis, hubiera sido un error tomar como controles pacientes exitosos de otras clínicas, que pudieran seguir protocolos de tratamiento diferentes, ya que entonces los controles no serían comparables a los casos.
...
* El núméro de casos activos diagnosticados de COVID-19 (su **prevalencia**, concepto que trataremos en detalle en la siguiente sección) era de algo menos de 2500 enfermos.
...
* Hay que tener en cuenta los **abandonos**: sujetos de la cohorte inicial a los que se les pierde la pista durante el seguimiento (en los estudios prospectivos) o sobre los que no se tiene información en el momento final del periodo de tiempo estudiado (en los estudios retrospectivos). Hay técnicas estadísticas específicas para tratarlos.
...
En un **estudio transversal** se toma un grupo de individuos (una **muestra transversal** de la población) y se observan simultáneamente, o  en un corto espacio de tiempo, la enfermedad y los factores de riesgo presentes en ese momento que puedan influir en la misma, u otras características de los individuos. El objetivo es inferir qué asociación hay entre estas condiciones  en la población general.
...
Una de las utilidades de los estudios transversales es poder estimar la **prevalencia** de una enfermedad (u otra característica): **la proporción, o número, de individuos con la enfermedad** en una población en un momento determinado. 
...
```{example,corsaib}
En el estudio ["Prevalencia de factores de riesgo cardiovascular en las Islas Baleares (estudio CORSAIB)"](https://www.revespcardiol.org/es-prevalencia-factores-riesgo-cardiovascular-islas-articulo-13082539) (F. Rigo Carratalà *et al*, *Revista Española de Cardiología* 58 (2005), pp. 1411-1419) se tomó una muestra aleatoria de habitantes de las Islas Baleares de la manera siguiente. Se dividió la comunidad en 14 sectores y participaron 3 o 4 médicos de familia por sector (50 en total). A continuación, se seleccionó aleatoriamente a 40 personas entre las adscritas a cada médico participante. A partir de las frecuencias de diversos factores de riesgo cardiovascular, se estimó su prevalencia en el total de nuestra comunidad. Por ejemplo:
...
Este ejemplo es un estudio transversal de prevalencia: se tomó una muestra **transversal** de la población (la que salió, sin especificar su composición según alguna clasificación de los individuos por factores de riesgo o enfermedades) y su objetivo era estimar, a partir de la muestra, la prevalencia de algunas características entre la población de  nuestra comunidad. 
...
Pues sí,  podríamos considerarlo como de casos y controles, aunque el hecho de anotar no solo si los sujetos tenían la enfermedades a dia 1/1/2017, sino también si las habían tenido en el pasado, no encaja del todo con un diseño de casos y controles. Pero como la exposición, es decir, el nacimiento, es anterior a cualquier enfermedad que pudieran desarrollar los sujetos, es aceptable.

Y seguramente también podríamos considerarlo de cohorte retrospectivo, aunque para clasificarlo como tal sería necesario que las enfermedades fueran tales que los sujetos no las tuvieran en el momento de nacer (lo fueron). Recordad que en los estudios de cohorte, en el momento en el que se clasifican los sujetos según su exposición a factores de riesgo han de estar **sanos**.
...
```{block2,type="rmdimportant"}
¿Cuál es entonces la respuesta correcta? La verdad es que cualquiera de las tres. Lo importante no es tanto poner una etiqueta a un estudio como razonar correctamente qué tipo de estudio "puede ser" para poder llevar a cabo luego el análisis estadístico  de los datos recogidos de manera consistente con el tipo elegido.
```
...
```{block2,type="rmdromans"}
¿De cohortes? ¿Y qué pasa con la condición de que los individuos han de estar "sanos" en el momento de clasificarlos en expuestos y no expuestos?
```
...
Pero de entre todos sus inconvenientes, el más destacable es que, al referir la información a un momento concreto,  a partir de un estudio transversal en principio **es imposible obtener ninguna información sobre relaciones temporales** tipo antecedente/desenlace. 

Si en una muestra de individuos anotamos, por ejemplo, si en ese momento tienen caries o no y si usan inhaladores para el asma o no, no obtenemos ninguna información sobre si alguna de esas dos condiciones es anterior a la otra y por lo tanto no podemos deducir deducir nada sobre si una podría ser susceptible de "causar" la otra.
...
* **Prevalencia de periodo**: número (o proporción) de **personas** que en algún momento de un periodo de tiempo concreto  han tenido la enfermedad, (contando los que ya estaban enfermos al inicio del periodo de tiempo considerado); si una persona la ha tenido varias veces, solo cuenta una vez en este valor. 
...
Está claro que la respuesta ha de ser (2) o (4); no es prevalencia puntual, porque no es el número de casos en un momento concreto sino durante un periodo de tiempo, y no es letalidad porque no se trata de la proporción de accidentes de tráfico que dan lugar a la muerte de algún involucrado (esa sería la **tasa de letalidad** de los accidentes de tráfico).
...
En principio, no. Este tipo de conclusión es lo que se conoce como **falacia ecológica**: trasladar una asociación positiva entre dos características a nivel de comunidades a una asociación positiva a nivel de individuos. 
...
¿Se puede concluir que consumir más carne roja aumenta el riesgo de cáncer de colon entre las mujeres? No, la tendencia observada es a nivel de países, no a nivel individual. Para poder deducir una asociación a nivel de mujeres, sería necesario un estudio de casos y controles o uno de cohorte.

¿Para qué sirve entonces un estudio ecológico? Básicamente, para aportar evidencia, aunque sea indirecta, sobre una asociación y sobre todo para generar hipótesis que luego se pueden estudiar más a fondo con otros estudios: estudios de casos y controles o de cohorte que relacionen el consumo de carne roja con el cáncer de colon, o la exposición a radón doméstico con el cáncer en pulmón. Como además son muy fáciles de llevar a cabo, ya que en general los datos en los que se basan son públicos en bases de datos al alcance de todos y se recogen en una tarde, son bastante comunes en epidemiología.
...
```{block2,type="rmdnote"}
Como comentaremos más adelante,  los estudios intervencionistas suelen aportar una mayor evidencia de la veracidad a nivel de población de los resultados obtenidos que los estudios observacionales. Pero a menudo es imposible llevarlos a cabo, o al menos no sería ético hacerlo. Por ejemplo, si queremos estudiar si la obesidad aumenta el riesgo de ingreso en UCI en caso de tener COVID-19, ¿cómo podríamos "intervenir" para que unos pacientes fueran obesos y otros no? O, retomando el ejemplo del radón y el cáncer de pulmón, a lo mejor a los nazis se les podría haber ocurrido exponer a propósito un grupo de internados en un campo de concentración al radón y otros no para ver si los primeros desarrollan más cáncer de pulmón que los segundos. Pero no sería de ninguna manera ético.
```
...
```{block2,type="rmdnote"}
**En teoría**, lo estudios experimentales permiten establecer una asociación causal entre las variables, porque **en teoría** el investigador controla completamente las características de los sujetos del estudio, de manera que las diferencias que se den entre los diferentes grupos solo puedan deberse a los diferentes tratamientos. Pero esto es solo **en teoría**, porque en la práctica es imposible controlarlo todo. 
```
...
* Los sujetos que **participan** en el estudio: aquellos a los que se administran (o no) los diferentes tratamientos.
...
*  **Grupos cruzados**: Los diferentes tratamientos son administrados a cada sujeto en momentos diferentes, en un orden elegido al azar  (para que haya aleatorización). 


```{r cruzados,echo=FALSE, out.width="80%",fig.cap="Esquema de un estudio con grupos cruzados."}
...
* Los **ensayos de fase II** se centran en la pregunta **¿Es efectivo el tratamiento?**. En ellos se prueba el fármaco sobre un grupo de enfermos de tamaño medio (digamos, de centenares) y se comparan los resultados con un grupo de control, pero posiblemente sin aleatorizar los tratamientos. Se estudian la eficacia del medicamento (relación dosis/respuesta, dosis óptima, margen de seguridad....) y los efectos adversos a corto plazo en diferentes condiciones "de laboratorio",  seguramente diferentes de lo que va a ser su administración "habitual" si se aprueba el fármaco.
...
Para terminar, el gráfico siguiente representa la llamada **pirámide de la evidencia**, que ordena los tipos de estudio, de la base hacia la cúspide, en función de la calidad de la evidencia científica que, en general, aportan, de manera que cuanto más arriba en la pirámide está un tipo de estudio, en principio con más seguridad se pueden adoptar sus conclusiones en la práctica médica. Si reflexionáis un poco, veréis que el orden que describe es muy razonable.
...
Seguramente no. Incluso los autores lo reconocen en las Conclusiones. En lo que se detectó una diferencia significativa fue en **el número de diagnósticos de glioma**, pero pudo deberse simplemente a que las personas con estudios superiores tienen una mayor probabilidad de que su glioma sea detectado, por ejemplo porque acudan al médico antes, y que sea registrado en el Registro Nacional de Cáncer, que es de donde se extrajo la información sobre los cánceres.  Naturalmente, en el periódico solo vais a encontrar la parte llamativa del estudio, como muestra el titular de la noticia del ABC sobre este estudio.
...
*(b)* Teniendo en cuenta que la tuberculosis era una de las causas principales de hospitalización en el hospital donde se llevaron a cabo las autopsias, ¿crees que su conclusión está justificada?
