...
hist(Medias,freq=FALSE,main="Histograma de las medias muestrales",xlab="",ylab="Densidad",col="light blue")
```
...
Por lo tanto, si $X$ es normal,  **esperamos** que la varianza muestral de una muestra aleatoria simple valga $\sigma_{X}^2$, en el sentido usual de que si tomamos muestras aleatorias simples de $X$ de tamaño $n$ grande y calculamos sus varianzas muestrales, esperamos de media obtener un valor muy cercano a $\sigma_{X}^2$.

```{block2,type="rmdcaution"}
Y por lo tanto **no esperamos** que la varianza "a secas" de una muestra aleatoria simple valga $\sigma_{X}^2$, porque la varianza muestral y la varianza "a secas" toman valores diferentes (tienen el mismo numerador y denominadores diferentes).
```

El segundo resultado nos dice que un  múltiplo adecuado de $\widetilde{S}_{X}^2$ tiene  distribución muestral conocida, lo que nos permite calcular la probabilidad de que le pase algo a $\widetilde{S}_{X}^2$.
...
A medida que el número de grados de libertad $\nu$ crece, la asimetría tiende a desaparecer y, por el Teorema Central del Límite, la distribución $\chi_\nu^2$ se aproxima a la de una variable normal $N(\nu,\sqrt{2\nu})$.
