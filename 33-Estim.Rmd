...
¿Qué calculamos? Pues un **estimador**: alguna función adecuada aplicada a los valores de la muestra, y que dependerá de la que queramos estimar.  Por ejemplo:
...
Cuando queremos estimar el valor medio de una variable sobre una población, tomamos una muestra de valores y calculamos su media aritmética, ¿verdad? Pues eso: 

Dada una variable aleatoria $X$, llamamos **media muestral** (**de tamaño $n$**) a la variable aleatoria $\overline{X}$ "Tomamos una muestra aleatoria de tamaño $n$ de $X$ y calculamos la media aritmética de sus valores".
...
* Si las muestras aleatorias son sin reposiciónm y $N$ es el tamaño de la población,
$$
...
La media muestral $\overline{X}$ de muestras aleatorias simples de tamaño $n$ de una variable aleatoria $X$ se  interpreta formalmente como la variable aleatoria obtenida tomando $n$ copias independientes $X_1,\ldots,X_n$ de $X$ y calculando
$$
...
```{theorem}
Si $X$ es $N(\mu_X,\sigma_X)$ y las muestras aleatorias son simples, entonces
$$
...
```{theorem}
Sea $X$ una variable aleatoria **cualquiera** de esperanza $\mu_X$ y desviación típica $\sigma_X$. Si las muestras aleatorias son simples y $n$ es suficientemente grande,
$$
...
* ¿Cuándo una muestra es suficientemente grane? En realidad, depende de la $X$. Cuánto más se parezca $X$ a una variable normal, más pequeñas pueden ser la muestras. Por fijar, un valor, aceptaremos que "suficientement grande" es $n\geq 30$.

* ¿Qué quiere decir que una variable aleatoria sea "aproximadamente" normal? Pues que su función de distribución $F_X$ toma valores muy cercanos a la función de distribución de una normal.
...
* $E(\widehat{p}_X)=p_X$
    ...
    Por lo tanto, $\widehat{p}_X$ es un estimador insesgado de $p_X$. Si calculáramos muchas proporciones muestrales de muestras aleatorias de $X$, es muy probable que, de media, obtuviéramos un valor muy cercano a  la proporción poblacional de éxitos $p_X$. 
...
```{theorem}
Si $X$ es $N(\mu_X,\sigma_X)$ y tomamos muestras aleatorias de tamaño $n$,
$$
...
```{theorem}
Si $X$ es $N(\mu_X,\sigma_X)$ y tomamos muestras aleatorias de tamaño $n$,La variable aleatoria 
$$
...
Aunque, por el Teorema Central del Límite, si $n$ es muy grande, la distribución $\chi_n^2$ se aproxima a la de una variable normal $N(n,\sqrt{2n})$.
...
1. Tenemos que aumentar en un 50% el tamaño de la muestra
1. Tenemos que doblar el tamaño  de la muestra.
1. Tenemos que cuadruplicar el tamaño  de la muestra.  
1. Tenemos que dividir por 2 el tamaño  de la muestra.
1. Tenemos que dividir por 4 el tamaño  de la muestra.
...
**(4)** Sobre una muestra de 100 mujeres  se obtuvo una concentración media de la hemoglobina de 10 con una desviación típica de 2. ¿Cuál es el error típico de la muestra (para la media muestral, se entiende)? 
...
1. Se calcula sumando todos los valores de la muestra y dividiendo por $n-1$, donde $n$ indica la medida de la muestra.
1. Ninguna de las otras respuestas es correcta.
...
1. El estadístico $\frac{\overline{X}-\mu}{\widetilde{S}_X/\sqrt{n}}$ sigue siempre una ley normal.
1. El estadístico $\frac{\overline{X}-\mu}{\widetilde{S}_X/\sqrt{n}}$ sigue siempre una ley t de Student.
1. El estadístico $\frac{\overline{X}-\mu}{\widetilde{S}_X/\sqrt{n}}$ sigue una ley normal si la concentración sigue una ley normal.
1. El estadístico $\frac{\overline{X}-\mu}{\widetilde{S}_X/\sqrt{n}}$ sigue una ley t de Student si la concentración sigue una ley normal.  
1. El estadístico $\frac{\overline{X}-\mu}{\widetilde{S}_X/\sqrt{n}}$ no sigue nunca ni una ley normal ni una ley t de Student, porque las muestras no son lo suficientemente grandes.
...
1. $\widetilde{S}_X^2$ sigue una distribución $\chi^2$ con $n-1$ grados de libertad.
1. $(n-1)\widetilde{S}_X^2/\sigma^2$ sigue una distribución $\chi^2$ con $n-1$ grados de libertad. 
