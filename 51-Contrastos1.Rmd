...
$$
Lo que sabemos calcular es
$$
P(\text{30 caras en 30 lanzamientos}\,|\,\text{La moneda es equilibrada})
$$
que vale $0.5^{30}=9.3\cdot 10^{-10}$ (y por lo tanto, de media, aproximadamente en una de cada mil millones de veces que se efectúan 30 lanzamientos seguidos de una moneda equilibrada, se obtienen 30 caras: no es imposible). 
...
$$
\text{p-valor}=P(S_{10}\leqslant 1)=0.01
$$
...
```{example}
Tenemos una moneda y nos preguntamos si está trucada; a favor de cara o a favor de cruz, nos da igual, solo si está trucada o es equilibrada.
...
\right.
$$
Supongamos que la lanzamos 10 veces y obtenemos 9 caras. ¿Es evidencia suficiente de que está trucada?
...
Nos preguntamos si los hombres jóvenes con diabetes tienen una concentración de calcio en plasma mayor que estos 2.5 mmol/l. Traducimos esta cuestión en un contraste de hipótesis sobre la concentración media de calcio en plasma en los hombres jóvenes con diabetes, a la que llamaremos $\mu$:
...
```{example,ttestmu20}
Sea $X$ una variable poblacional normal. Queremos realizar el contraste
$$
\left\{\begin{array}{l}
...
* El estadístico de contraste que usaremos es 
$$
T=\dfrac{\overline{X}-\mu_0}{\widetilde{S}_X/\sqrt{n}}
$$
con $\mu_0=20$ y $n=25$.

* Su valor en nuestra muestra es
$$
...
* El p-valor es
$$
P(T\geqslant 1.944)=P(t_{24}\geqslant 1.944)=0.032
$$
...
* p-valor
$$
P(T\leqslant 1.944)=P(t_{24}\leqslant 1.944)=0.968
$$
...
Vamos a ver, ¿cómo queríais que hubiéramos encontrado evidencia de que $\mu<20$ si nos ha salido una media muestral 20.7, mayor que 20? No hacía falta hacer ningún cálculo (y exponernos a equivocarnos), bastaba razonar un poco.
```
...
Ahora el p-valor es
$$
2\cdot P(T\geqslant 1.944)=2\cdot P(t_{24}\geqslant 1.944)=0.064
$$
...
* El p-valor es
$$
2P(T\geqslant -2.778)=P(t_{24}\geqslant -2.778)=1.99
$$
...
$$
2\cdot P(T\geqslant 2.778)=2\cdot P(t_{24}\geqslant 2.778)=0.01
$$
y como este p-valor es menor que $\alpha$, podemos rechazar $H_0$ y concluir, con nivel de significación 0.05, que $\mu\neq 20$. Es decir,  hemos obtenido evidencia estadísticamente significativa de que $\mu\neq 20$.


## La potencia de un contraste
...
Ambos métodos son equivalentes, porque
$$
p< \alpha/M\Longleftrightarrow M\cdot p<\alpha
$$
...
```{block2,type="rmdnote"}
Recordad que un p-valor es una probabilidad, por lo tanto no puede pasar de 1. Si al ajustar algún p-valor pasa de 1, se da 1 como resultado del ajuste.
```
...
Por ejemplo, imaginad que queremos estudiar si una vacuna para una determinada enfermedad es efectiva. Tomamos dos grupos de personas, vacunamos las de un grupo, dejamos sin vacunar las del otro, esperamos un tiempo prudencial, y contamos cuántos han enfermado en cada grupo. Fijemos el nivel de significación usual $\alpha=0.05$ y supongamos que obtenemos un p-valor por debajo de 0.05.
...
````{example}
En [un artículo publicado en 2010](https://www.sciencedirect.com/science/article/abs/pii/S004113451001081X) se obtuvo evidencia de que una persona que fuera Aries, Tauro, Géminis, Leo, Escorpio o Capricornio tenía una mayor probabilidad de supervivencia a 5 años vista tras un alotransplante de células madre para tratar una leucemia mieloide crónica  que una persona de fuera de los otros signos del zodíaco (58% de tasa de supervivencia frente a un 48%, p-valor 0.007). ¿Nos lo vamos a creer? Ya lo dicen los autores en el subtítulo del artículo: *An artificial association*.
...
1. Es la variable *h*aleatoria poblacional de interés
...
1. H~0~: El individuo tiene COVID-19; H~1~: El individuo no tiene COVID-19
1. H~0~: El individuo no tiene COVID-19; H~1~: El individuo tiene COVID-19 
1. H~0~: El individuo no tiene COVID-19; H~1~: El individuo podría tener COVID-19
1. H~0~: No sé si el individuo tiene COVID-19; H~1~: El individuo tiene COVID-19
...
1.  El estudio no permite concluir nada, ya que si realizamos 20 contrastes con un nivel de significación del 5%, esperamos que alguno dé un resultado estadísticamente significativo aunque no haya diferencia entre los tratamientos
