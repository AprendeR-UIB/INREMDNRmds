...
En muchas situaciones, queremos tomar una **decisión** sobre si podemos aceptar o rechazar una **hipótesis** sobre el valor de un parámetro en una o varias poblaciones, y para tomar esta decisión, nos basamos en los datos de una muestra. Por ejemplo:
...
```{example,juicio}
En un juicio (en el que el acusado es inocente si no se demuestra lo contrario, es decir, que estamos dispuestos a aceptar por defecto que es inocente), se busca evidencia de que el acusado es culpable. Por lo tanto, esta es la hipótesis alternativa. Así:
...
* Si estuviera totalmente a la izquierda del 0, con un 95% de confianza podríamos concluir que $\mu_m<\mu_h$ (porque tendríamos un 95% de seguridad de que el valor real de la diferencia $\mu_m-\mu_h$ pertenece a un intervalo de números estrictamente negativos).
...
Aquí la variable aleatoria $X$ que nos interesa es "lanzamos la moneda y miro si sale cara", que es Bernoulli con probabilidad de éxito (es decir, probabilidad de sacar cara con nuestra moneda) $p_{\mathit{Cara}}$.
...
\right.
$$
Suponemos que la lanzamos 10 veces y obtenemos 8 caras. ¿Es evidencia suficiente de que está trucada?
...
```{block2,type="rmdexercici"}
Recordad la interpretación de una prueba diagnóstica como un contraste de hipótesis (Ejemplo \@ref(exm:CHdiag)). Interpretad su especificidad y sensibilidad en términos de nivel de significación y potencia.
...
1. Se da una regla de decisión para un $\alpha$ máximo fijado.

2. Después, se toma el tamaño $n$ adecuado de la muestra para llegar a la $\beta$ deseada.
...
Tomando $\alpha=0.05$, aceptamos una probabilidad de equivocarnos rechazando $H_0$ en favor de $H_1$ de 0.05. Es decir, nos permitimos cometer, de media, un error de tipo I una vez de cada 20 que la hipótesis nula sea verdadera.
...
La definición precisa de "muy grande" dependerá del valor de $\alpha$ que queramos tomar, es decir, de la probabilidad de cometer un error de tipo I que estemos dispuestos a asumir: cuanto menor queramos que sea $\alpha$, mayor tendrá que ser $T$. Aquí vamos a tomar el valor usual $\alpha=0.05$.
...
> Hemos encontrado evidencia estadísticamente significativa de que el nivel medio de calcio en plasma en los jóvenes diabéticos es mayor que en los jóvenes sanos.  
...
T=\frac{\overline{X}-\mu_0}{{\widetilde{S}_X}/{\sqrt{n}}}
$$
sigue una distribución t de Student con $n-1$ grados de libertad, $t_{n-1}$,  se le llama un **test t**.
```
...
Ahora, repetimos 200 veces el proceso de tomar una muestra aleatoria de tamaño 20 de nuestra población y calcular la $T$ correspondiente. Llamamos `Tes` al vector de estos valores de $T$:
...
Hemos cometido un `r 100*p.error.Tipo.I`% de errores de tipo I.
...
¿Y qué pasa si ahora buscamos evidencia de que $\mu$ **es diferente** de $\mu_0$? Es decir, si tenemos el contraste
$$
\left\{\begin{array}{l}
H_{0}:\mu=\mu_0\\ 
H_{1}:\mu\ 
...
Fijaos en que, en realidad, lo que pasa es que encontraríamos evidencia de que $\mu \neq 20$ si $T$ fuera muy grande o muy pequeño. Por lo tanto, en el contraste bilateral tenemos dos fuentes de error de tipo I: que por puro azar $T$ nos salga muy grande o que nos salga muy pequeño.  En cambio, solo encontraremos evidencia de que $\mu> 20$ si $T$ es muy grande, y por tanto en el contraste unilateral tenemos una sola fuente de error de tipo I. Entonces, para garantizar una misma probabilidad de error de tipo I, tenemos que ser mucho más exigentes en el contraste bilateral, donde nos podemos equivocar de dos maneras diferentes, que en el unilateral. Por eso es más fácil rechazar la hipótesis nula en un contraste unilateral que en uno bilateral.
...
$$
¿Os suena? Llamando $q$ a $1-\alpha$, es el del tema anterior.
...
```{block2,type="rmdimportant"}
El p-valor obtenido en un contraste es la probabilidad mínima que tenemos de equivocarnos rechazando la hipótesis nula si esta es verdadera.
```
...
$$
1-\beta=P(\text{Rechazar } H_0| H_1\text{ verdadera})=P(T>1.73| \mu>2.5).
$$
...
Esta relación permite calcular cualquiera de los cuatro valores a partir de los otros tres. No entraremos en el detalle de cómo, pero al menos permitidnos mencionar que, con R, el paquete **pwr** aporta funciones que permiten hacerlo para los contrastes más usuales.
...
El paquete **statcheck** de R permite revisar de manera automática todos los cálculos de un artículo escrito en un formato concreto en psicología y comprobar los p-valores. Los autores del paquete analizaron 30,000 artículos y concluyeron que (*Behavior research methods* 48 (2016), 1205-1226):
...
**(1)** ¿Qué significa que en un contraste de hipótesis tomemos un nivel de significación del 1%? Marcad la respuesta correcta:
...
**(2)**  Al analizar los resultados de un ensayo clínico, se concluye que la tasa de curación de los dos tratamientos estudiados son diferentes, con $p=0.034$. Esto significa (marcad todas las respuestas correctas):
...
la diferencia no resultó estadísticamente significativa. Podemos concluir que (marcad todas las respuestas correctas):
...
que fueron muy pequeños al nacer se les dio la leche materna de su madre o leche infantil, a elección de la madre. A la edad de 8 años, se midió el CI (cociente intelectual) de estos niños. La media de CI en el grupo de  leche infantil fue 92.8, en comparación con una media de 103.0 en el grupo de leche materna. La diferencia fue significativa, $p <0.001$. Marcad todas  las afirmaciones correctas:
...
1. Si la elección del tipo de leche estuviera relacionada con el CI posterior, la probabilidad de que 
la diferencia observada en el CI medio fuera  la observada, o menor, es menor que 0.001. 
1. Todas las otras respuestas son incorrectas.  
...
1.  El estudio no permite concluir nada, ya si realizamos 20 contrastes con un nivel de significación del 5%, esperamos que uno dé un resultado estadísticamente significativo aunque no haya diferencia entre los tratamientos
