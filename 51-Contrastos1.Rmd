...
Si rechazamos $H_0$ en favor de $H_1$ **no** será porque hayamos demostrado que $H_0$ sea imposible, ni siquiera que sea improbable: tan solo habremos observado que es difícil de creer que sea verdad vistos los resultados de nuestro experimento.
```
...
```{example,esport1}
Si leemos la noticia siguiente en el diario, quede que nos preguntemos si es verdad que las mujeres practican  menos deporte que los hombres.
...
\right.
$$
y obtenemos que $\overline{X}_m$ es mucho mayor que $\overline{X}_h$ en nuestra muestra, concluimos que no tenemos evidencia que $\mu_m<\mu_h$ y punto. **Sería hacer trampas** decir: "No hemos encontrado evidencia de que las mujeres practiquen menos deporte que los hombres, pero si con estos mismos datos realizamos el contraste
$$
\left\{\begin{array}{ll} 
H_{0}: \mu_m=\mu_h\\ 
H_{1}:\mu_m>\mu_h
...
A este tipo de procedimiento, usar la distribución binomial del número de éxitos en una muestra aleatoria simple para contrastar un valor de la probabilidad poblacional de éxito, lo llamaremos un **test binomial**.
```
...
\right.
$$
Suponemos que la lanzo 10 veces y obtengo 8 caras. ¿Es evidencia suficiente de que está trucada?
...
El problema es que cuando hacemos que $\alpha$ disminuya, $\beta$ suele aumentar, porque al hacer más difícil rechazar la hipótesis nula, aumenta el riesgo de no rechazarla "cuando toque".
...
$$
Como el p-valor es menor que 0.05, concluimos con un nivel de significación del 5% que el nivel medio de calcio en plasma en los jóvenes diabéticos es mayor que en los jóvenes sanos.
...
```{block2,type="rmdrecordau"}
A este tipo de procedimiento, usando que de 
$$
T=\frac{\overline{X}-\mu_0}{{\widetilde{S}_X}/{\sqrt{n}}}
$$
sigue una distribución t de Student con $n-1$ grados de libertad, $t_{n-1}$, para comparar la $\mu$ de una variable con el valor $\mu_0$ lo llamaremos un **test t**.
```
...
¡Hemos cometido exactamente un `r 100*p.error.Tipus.I`% de errores de tipo I!
...
La proporción media de errores de tipo II ha sido:
...
Veamos, ¿cómo queríais que concluyéramos que $\mu<20$ si nos ha salido una media muestral 20.7, mayor que 20? No hacía falta hacer ningún cálculo (y exponernos a equivocarnos), bastaba razonar un poco.
```
...
$$
Si la $\mu_0$ que contrastamos pertenece a este intervalo, no podemos concluir que la $\mu$ poblacional sea más grande, y por tanto no podemos rechazar que $\mu=\mu_0$.
...
$$
¿Os suena? Haciendo $q=1-\alpha$, es el del tema anterior.
...
```{block2,type="rmdimportant"}
El p-valor obtenido en un contraste es la probabilidad mínima que tenemos de equivocarnos rechazando la hipótesis nula si es verdadera.
```
...
Por ejemplo, en el ejemplo del calcio en diabéticos de la Sección \@ref(seco:exttest), la regla de rechazo era 
$$
...
$$
Esta probabilidad es imposible de calcular, pero hay programas que la saben estimar.
...
```{block2,type="rmdcaution"}
Desconfiad de los trabajos donde esto no se haga. Podría ser que la potencia fuera muy baja y hubiera un **sesgo de infrapotència** (*underpower*): se necesitaba un efecto muy grande para poder rechazar la hipótesis nula y publicar el artículo.
```
...
El **riesgo de falso positivo**, *FPR**, en un contraste es
$$
P(H_0\text{ verdadera}|H_0\text{ rechazada}).
$$
Por el teorema de Bayes (notad que interpretamos $H_1= \text{no }H_0$)
$$
\begin{array}{rl}
...
$$
Por lo tanto, *a posteriori*, creo que hay un 47% de probabilidades que $H_1$ sea falsa y un 53% de probabilidades que $H_1$ sea verdadera.
