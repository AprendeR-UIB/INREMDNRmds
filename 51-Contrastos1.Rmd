...
* Si obtenemos evidencia suficiente de que $H_0$ es falsa, rechazaremos $H_0$ en favor de $H_1$ y concluiremos que $H_1$ es verdadera. 
    ...
    * Tendremos evidencia de que la moneda está trucada a favor de cara si en nuestra serie de lanzamientos la proporción de caras es tan grande que se nos hace muy difícil creer que la moneda no esté trucada a favor de cara.
...
Observad que considerar no culpable no es lo mismo que demostrar que es inocente: simplemente, se considera que el acusado no es culpable porque no se ha encontrado  evidencia suficiente de que lo sea.
...
```{block2,type="rmdnote"}
Observad el razonamiento que hemos efectuado. Tenemos una hipótesis nula y una de alternativa, realizamos un experimento  y obtenemos un resultado que es muy improbable si la hipótesis nula es verdadera. Una de dos:
...
2. Después, se toma el tamaño $n$ adecuado de la muestra para que la la potencia sea la deseada.
...
Tomando este $\alpha=0.05$, aceptamos una probabilidad de equivocarnos rechazando $H_0$ en favor de $H_1$ de 0.05. Es decir, asumimos que, de media, nos vamos a equivocar 1 de cada 20 veces en que la hipótesis nula sea verdadera.
...
Se han propuesto muchos métodos para ajustar los p-valores de grupos de contrastes. El más sencillo, y el más popular, es el **de Bonferroni**. Su idea es que, usando la aproximación $1-(1-x)^M \approx M\cdot x$, si queremos efectuar $M$ contrastes con un nivel de significación global $\alpha$:
...
Por ejemplo, si realizamos 3 contrastes con nivel de significación $\alpha =0.05$, para obtener un nivel de significación global $\alpha =0.05$ el método de Bonferroni nos dice que tenemos qu multiplicar cada p-valor por 3.
...
Esta relación permite calcular cualquiera de los cuatro valores a partir de los otros tres. Así, sabiendo $n$, $\alpha$ y el tamaño del efecto obtenido, podemos calcular la potencia del contraste efectuado.
...
$$
Si la $\mu_0$ que contrastamos pertenece a este intervalo, no podemos concluir que la $\mu$ poblacional sea mayor que $\mu_0$, y por tanto no podemos rechazar que $\mu=\mu_0$. 
