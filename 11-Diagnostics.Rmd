...
* **Test diagnóstico**: se realiza en individuos que muestran síntomas o que han dado positivo en un test de cribado, el objetivo principal de detectar la presencia de la enfermedad.
...
*(b)* En primer lugar vamos a calcular los valores predictivos usando la fórmula de Bayes. Como $P(+|E)=0.94$, $P(-|S)=14/15$ y en Uruguay $P(E)=1/64$, por lo que $P(S)=53/64$:
...
Notas que, en España,  la prevalencia es más pequeña, el VPP es más pequeño y el VPN más grande. El comportamiento de los valores predictivos es razonable:
...
* Hay menos sanos, por lo que aumenta la probabilidad de encontrar sanos y por lo tanto  el VPN es menor.

```{block2,type="rmdcorbes"}
El motivo matemátic es el siguiente. Si llamamos $x\in [0,1]$ a la prevalencia y fijamos los valores de $P(+|E),P(+|S),P(-|E),P(-|S)$, de manera que el VPP y el VPN sean una función solo de $x$, tenemos que
$$
\begin{array}{l}        
...
Una de las aplicaciones de la curva ROC es ayudar a decidir qué valor de corte tomar para definir el test diagnóstico binario que se va a usar en la práctica.  Una estrategia muy común es tomar el **valor de corte óptimo**: el valor $x_o$ el que la exactitud de la prueba,
$$
