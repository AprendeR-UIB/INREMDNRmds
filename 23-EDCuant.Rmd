...
## Datos cuantitativos bivariantes {#sec:edbivar}
...
$$
Estudiaremos con más detalles esta recta de regresión en la Sección \@ref(sec:rls).
...
```{r jamoviedc5.5,echo=FALSE,out.width="100%"}
knitr::include_graphics("INREMDN_files/figure-html/JAMOVI.EDC.5.5.png")
```
...
```{example, Anscombe} 
F. Anscombe produjo los cuatro conjuntos de puntos descritos en la Figura \@ref(fig:anscombe). Como podéis, ver, tienen ajustes muy diferentes a una recta, y en cambio resulta que tienen el mismo valor de $r$: 0.816regres. Para más detalles, consultad la [correspondiente entrada de la Wikipedia](https://es.wikipedia.org/wiki/Cuarteto_de_Anscombe).
```
